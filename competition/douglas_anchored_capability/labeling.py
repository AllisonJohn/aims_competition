"""Adaptive label acquisition for anchored capability calibration."""
...
try:
    ...
    ARTIFACT = {"global_rate": 0.5, "subject_rates": {}, "condition_rates": {}, "benchmark_rates": {}}

GLOBAL_RATE = float(ARTIFACT.get("global_rate", 0.5))
SUBJECT_RATES = ARTIFACT.get("subject_rates", {})
CONDITION_RATES = ARTIFACT.get("condition_rates", {})
BENCHMARK_RATES = ARTIFACT.get("benchmark_rates", {})
...
def _complexity_bonus(item_content: object) -> float:
...
def acquisition_function(input: dict) -> float:
    ...
    return float(fisher * (0.75 + 0.25 * reliability) + _complexity_bonus(input) + 1e-4 * _jitter(input))
