"""Empirical-Bayes prior submission.

This variant deliberately avoids embeddings/regression. It combines smoothed
subject, condition, benchmark, and subject-by-capability bucket rates.
"""
...
with (ROOT / "artifacts" / "empirical_bayes_artifact.json").open("r", encoding="utf-8") as f:
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    capability_delta = _logit(subject_bucket_rate) - _logit(bucket_rate)

    z = (
        float(WEIGHTS["subject"]) * _logit(subject_rate)
        + float(WEIGHTS["condition"]) * _logit(condition_rate)
        + float(WEIGHTS["benchmark"]) * _logit(benchmark_rate)
        + float(WEIGHTS["capability"]) * capability_delta
        + float(WEIGHTS["item_adjustment"]) * _item_adjustment(input.get("item_content"))
    )
