...
app = modal.App("cs321m-final-factorization-machine")

image = (
    modal.Image.debian_slim(python_version="3.11")
    .pip_install("numpy", "pandas", "pyarrow", "scikit-learn", "torch", "huggingface_hub", "hf_xet")
)
...
def train_remote(
    ...
    n, d = x_tensor.shape
    linear = torch.nn.Parameter(torch.zeros(d))
    factors = torch.nn.Parameter(torch.randn(d, latent_dim) * 0.01)
    bias = torch.nn.Parameter(torch.tensor(float(np.log(global_rate / (1.0 - global_rate)))))
    optimizer = torch.optim.AdamW([bias, linear, factors], lr=learning_rate, weight_decay=weight_decay)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for start in range(0, n, batch_size):
            ...
            xb = x_tensor[indices]
            yb = y_tensor[indices]
            linear_term = xb @ linear
            summed = xb @ factors
            summed_square = summed * summed
            square_summed = (xb * xb) @ (factors * factors)
            interactions = 0.5 * (summed_square - square_summed).sum(dim=1)
            logits = bias + linear_term + interactions
            ...
            seen += len(indices)
        print(f"fm_epoch={epoch} train_bce={running / max(1, seen):.6f}", flush=True)

    with torch.no_grad():
        linear_term = x_tensor @ linear
        summed = x_tensor @ factors
        interactions = 0.5 * ((summed * summed) - ((x_tensor * x_tensor) @ (factors * factors))).sum(dim=1)
        logits = bias + linear_term + interactions
        train_loss = float(torch.nn.functional.binary_cross_entropy_with_logits(logits, y_tensor).item())
        train_accuracy = float(((torch.sigmoid(logits) >= 0.5) == (y_tensor >= 0.5)).float().mean().item())
    ...
    artifact = {
        ...
        "linear": linear.detach().cpu().numpy().astype(float).tolist(),
        "factors": factors.detach().cpu().numpy().astype(float).tolist(),
...
def main(
    ...
    (out_dir / "factorization_machine_artifact.json").write_text(
        json.dumps(payload["artifact"], separators=(",", ":"), sort_keys=True),
        encoding="utf-8",
    )
    (out_dir / "factorization_machine_summary.json").write_text(
