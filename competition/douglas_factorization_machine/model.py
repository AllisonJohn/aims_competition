"""Factorization-machine prior/text interaction submission."""
...
with (ROOT / "artifacts" / "factorization_machine_artifact.json").open("r", encoding="utf-8") as f:
...
LINEAR = [float(x) for x in ARTIFACT["linear"]]
FACTORS = [[float(x) for x in row] for row in ARTIFACT["factors"]]
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    x = _features(input)
    z = BIAS + sum(weight * value for weight, value in zip(LINEAR, x))
    for k in range(LATENT_DIM):
        summed = 0.0
        square_summed = 0.0
        for value, row in zip(x, FACTORS):
            product = value * row[k]
            summed += product
            square_summed += product * product
        z += 0.5 * (summed * summed - square_summed)
