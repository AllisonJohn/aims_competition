"""Gradient-boosted tree prior/text-feature submission."""
...
with (ROOT / "artifacts" / "gbdt_artifact.json").open("r", encoding="utf-8") as f:
...
MODEL = pickle.loads(base64.b64decode(ARTIFACT["model_pickle_b64"]))
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    x = _features(input)
    prediction = _clamp(float(MODEL.predict_proba([x])[0][1]))
