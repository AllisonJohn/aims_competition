...
Run:
    modal run competition/douglas_ridge/modal_validate_bge_ridge.py
"""
...
APP_NAME = "douglas-ridge-validation"
...
image = (
    ...
    .add_local_dir(LOCAL_RIDGE_DIR, remote_path="/root/competition/douglas_ridge")
)
...
def validate_remote(
    ...
    prior_weight_grid: str = "0.68,0.22,0.10;0.75,0.15,0.10;0.80,0.10,0.10;0.90,0.10,0.00;1.00,0.00,0.00;0.60,0.25,0.15;0.50,0.30,0.20",
    item_adjustment_weights: str = "0,0.5,1.0,1.5",
) -> dict:
    ...
    def parse_prior_weight_grid(value: str) -> list[dict[str, float]]:
        rows = []
        for row in value.split(";"):
            parts = [float(part.strip()) for part in row.split(",") if part.strip()]
            if len(parts) != 3:
                continue
            rows.append(
                {
        ...
        return rows or [{"subject": 0.68, "condition": 0.22, "benchmark": 0.10}]
    ...
    remote_submission_dir = Path("/root/competition/douglas_ridge")
    ...
    for row in valid_df.itertuples(index=False):
        ...
        subject_name = model_module._parse_subject_name(input_row.get("subject_content"))
        condition = model_module._key(input_row.get("condition") or "none")
        benchmark = model_module._key(input_row.get("benchmark"))
        subject_rate = model_module._smooth(
            model_module.SUBJECT_RATES.get(subject_name),
            model_module.SUBJECT_ALPHA,
        )
        condition_rate = model_module._smooth(
            model_module.CONDITION_RATES.get(condition),
            model_module.CONDITION_ALPHA,
        )
        benchmark_rate = model_module._smooth(
            model_module.BENCHMARK_RATES.get(benchmark),
            model_module.BENCHMARK_ALPHA,
        )
        base_components.append(
            {
                "subject_logit": model_module._logit(subject_rate),
                "condition_logit": model_module._logit(condition_rate),
                "benchmark_logit": model_module._logit(benchmark_rate),
                "item_adjustment": model_module._item_adjustment(input_row.get("item_content")),
            }
        )
    ...
    sweep = []
    prior_grids = parse_prior_weight_grid(prior_weight_grid)
    adjustment_grids = parse_float_list(item_adjustment_weights)
    for prior_weights in prior_grids:
        for item_adjustment_weight in adjustment_grids:
            for weight in parse_blend_weights(blend_weights):
                predictions = []
                for components, difficulty in zip(base_components, difficulties):
                    base_logit = (
                        prior_weights["subject"] * components["subject_logit"]
                        + prior_weights["condition"] * components["condition_logit"]
                        + prior_weights["benchmark"] * components["benchmark_logit"]
                        + item_adjustment_weight * components["item_adjustment"]
                    )
                    base = model_module._clamp(model_module._sigmoid(base_logit))
                    if difficulty is None:
                        prediction = base
                    else:
                        prediction = model_module._sigmoid(model_module._logit(base) - weight * difficulty)
                ...
                sweep.append(
                    {
                        ...
                        "prior_weights": {
                            "subject": float(prior_weights["subject"]),
                            "condition": float(prior_weights["condition"]),
                            "benchmark": float(prior_weights["benchmark"]),
    ...
    best = max(sweep, key=lambda row: row["negative_log_loss"])
    default = min(
        sweep,
        key=lambda row: (
            abs(row["blend_weight"] - 0.45)
            + abs(row["prior_weights"]["subject"] - 0.68)
            + abs(row["prior_weights"]["condition"] - 0.22)
            + abs(row["prior_weights"]["benchmark"] - 0.10)
            + abs(row["prior_weights"]["item_adjustment"] - 1.0)
        ),
    )
    top_sweep = sorted(sweep, key=lambda row: row["negative_log_loss"], reverse=True)[:20]
...
def main(
    ...
    payload = validate_remote.remote(
        ...
        item_adjustment_weights=item_adjustment_weights,
    )
    summary_path = LOCAL_ARTIFACT_DIR / "validation_80_20_summary.json"
