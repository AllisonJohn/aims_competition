"""2PL BGE ridge learned-skip submission.

Training writes ``artifacts/bge_2pl_ridge_artifact.json``. At prediction time
we embed the item, ridge-predict item difficulty and discrimination, then let
...
with (ROOT / "artifacts" / "baseline_stats.json").open("r", encoding="utf-8") as f:
    STATS = json.load(f)
with (ROOT / "artifacts" / "bge_2pl_ridge_artifact.json").open("r", encoding="utf-8") as f:
...
DIFFICULTY_INTERCEPT = float(ARTIFACT["difficulty_ridge_intercept"])
DIFFICULTY_COEF = [float(x) for x in ARTIFACT["difficulty_ridge_coef"]]
LOG_DISC_INTERCEPT = float(ARTIFACT["log_discrimination_ridge_intercept"])
LOG_DISC_COEF = [float(x) for x in ARTIFACT["log_discrimination_ridge_coef"]]
...
def _predict_item_2pl(item_content: object) -> tuple[float, float] | None:
    ...
    if features is None:
        return None
    difficulty = DIFFICULTY_INTERCEPT + sum(weight * value for weight, value in zip(DIFFICULTY_COEF, features))
    log_disc = LOG_DISC_INTERCEPT + sum(weight * value for weight, value in zip(LOG_DISC_COEF, features))
    log_disc += GLOBAL_LOG_DISC
...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    ...
    for example in labeled:
        ...
        expected.append(predict(example, None))
    if not labels:
        return prediction
