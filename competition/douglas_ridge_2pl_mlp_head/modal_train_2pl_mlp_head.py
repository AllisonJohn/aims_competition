...
app = modal.App("cs321m-final-bge-2pl-mlp-head")
...
def train_remote(
    ...
    y_item = np.stack(
        [
            item_targets["centered_difficulty"].to_numpy(dtype=np.float32),
            item_targets["centered_log_discrimination"].to_numpy(dtype=np.float32),
        ],
        axis=1,
    )
    item_target_scaler = StandardScaler()
    y_item_scaled = item_target_scaler.fit_transform(y_item).astype(np.float32)
    ...
    item_y = torch.from_numpy(y_item_scaled)
    item_generator = torch.Generator().manual_seed(326)
    print(
        f"Training item MLP head: items={len(item_x)} input_dim={x_train.shape[1]} "
    ...
    for epoch in range(1, int(item_head_epochs) + 1):
        permutation = torch.randperm(len(item_x), generator=item_generator)
        total_loss = 0.0
        total_examples = 0
        for start in range(0, len(item_x), int(item_head_batch_size)):
            idx = permutation[start : start + int(item_head_batch_size)]
            batch_x = item_x[idx].to(device)
            batch_y = item_y[idx].to(device)
            item_optimizer.zero_grad(set_to_none=True)
            prediction = item_head(batch_x)
        ...
        if epoch == 1 or epoch % 5 == 0 or epoch == int(item_head_epochs):
            print(f"item_head_epoch={epoch} mse={total_loss / max(total_examples, 1):.6f}", flush=True)

    item_head.eval()
    with torch.no_grad():
        item_predictions = item_head(item_x.to(device)).detach().cpu().numpy().astype(np.float32)
    item_predictions = item_target_scaler.inverse_transform(item_predictions).astype(np.float32)
    item_targets["difficulty_hat"] = item_predictions[:, 0]
    item_targets["log_discrimination_hat"] = item_predictions[:, 1] + global_log_discrimination
    ...
    return {
        "artifact": {
            ...
            "item_head": {
                ...
                "output_dim": 2,
                "target_mean": [float(x) for x in item_target_scaler.mean_],
                "target_scale": [float(x) if float(x) != 0.0 else 1.0 for x in item_target_scaler.scale_],
...
def main(
    ...
    (out_dir / "bge_2pl_mlp_artifact.json").write_text(
        json.dumps(payload["artifact"], separators=(",", ":"), sort_keys=True),
        encoding="utf-8",
    )
    (out_dir / "bge_2pl_mlp_summary.json").write_text(
        json.dumps(payload["summary"], indent=2, sort_keys=True),
        encoding="utf-8",
    )
    print(json.dumps(payload["summary"], indent=2, sort_keys=True))
    print(f"Wrote {out_dir / 'bge_2pl_mlp_artifact.json'}")
