...
app = modal.App("cs321m-final-bge-bayesian-irt-artifact")
...
def train_remote(
    ...
    bayes_item_l2: float = 1.0,
    bayes_subject_l2: float = 0.1,
    bayes_epochs: int = 8,
) -> dict:
    import math
    ...
    y_train = item_targets["difficulty"].to_numpy(dtype=np.float32)
    model = Ridge(alpha=300.0, random_state=326)
    model.fit(x_train, y_train)
    item_targets["difficulty_prior_hat"] = model.predict(x_train).astype(np.float32)

    item_index = {str(item_key): idx for idx, item_key in enumerate(item_uniques)}
    beta_prior = np.zeros(len(item_uniques), dtype=np.float64)
    for row in item_targets.itertuples(index=False):
        idx = item_index.get(str(row.item_key))
        if idx is not None:
            beta_prior[idx] = float(row.difficulty_prior_hat)

    theta_map = theta.copy()
    beta_map = beta_prior.copy()
    for epoch in range(max(1, int(bayes_epochs))):
        eta = theta_map[subject_codes] - beta_map[item_codes]
        p = sigmoid(eta)
        for i, idx in enumerate(subj_groups):
            if len(idx) == 0:
                continue
            grad = (y[idx] - p[idx]).sum() - bayes_subject_l2 * theta_map[i]
            hess = -(p[idx] * (1 - p[idx])).sum() - bayes_subject_l2
            theta_map[i] -= grad / hess
        theta_map -= theta_map.mean()
        eta = theta_map[subject_codes] - beta_map[item_codes]
        p = sigmoid(eta)
        for j, idx in enumerate(item_groups):
            if len(idx) == 0:
                continue
            grad = -(y[idx] - p[idx]).sum() - bayes_item_l2 * (beta_map[j] - beta_prior[j])
            hess = -(p[idx] * (1 - p[idx])).sum() - bayes_item_l2
            beta_map[j] -= grad / hess
        eta = theta_map[subject_codes] - beta_map[item_codes]
        p = sigmoid(eta)
        bce = -np.mean(y * np.log(np.clip(p, 1e-6, 1 - 1e-6)) + (1 - y) * np.log(np.clip(1 - p, 1e-6, 1 - 1e-6)))
        print(f"bayes_irt_epoch={epoch + 1} train_bce={bce:.6f}", flush=True)

    posterior_by_item = dict(zip([str(x) for x in item_uniques], beta_map))
    item_targets["posterior_difficulty"] = item_targets["item_key"].astype(str).map(posterior_by_item).fillna(
        item_targets["difficulty_prior_hat"]
    )
    y_train = item_targets["posterior_difficulty"].to_numpy(dtype=np.float32)
    model.fit(x_train, y_train)
    ...
    return {
        "artifact": {
            ...
            "bayesian_irt": {
                "item_l2": float(bayes_item_l2),
                "subject_l2": float(bayes_subject_l2),
                "epochs": int(bayes_epochs),
                "prior_source": "bge_ridge_raw_rasch_difficulty",
                "posterior_target": "map_item_difficulty",
            },
            "prior_weights": {
