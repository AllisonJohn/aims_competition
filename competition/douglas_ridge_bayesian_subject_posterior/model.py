...
At prediction time, labeled examples update a Gaussian prior over subject
capability with a one-dimensional IRT MAP posterior.
"""
...
POSTERIOR_PRIOR_PRECISION = float(ARTIFACT.get("posterior_prior_precision", 1.5))
POSTERIOR_MAX_STEPS = int(ARTIFACT.get("posterior_max_steps", 8))
POSTERIOR_MAX_LABELS = int(ARTIFACT.get("posterior_max_labels", 32))
...
def _ability_prior_logit(input: dict) -> float:
    subject_name = _parse_subject_name(input.get("subject_content"))
    condition = _key(input.get("condition") or "none")
    benchmark = _key(input.get("benchmark"))
    subject_rate = _smooth(SUBJECT_RATES.get(subject_name), SUBJECT_ALPHA)
...
def _posterior_subject_prediction(
    prediction: float,
    input: dict,
    ...
    for example in labeled:
        ...
        if difficulty is None:
            difficulty = 0.0
        offset = ITEM_ADJUSTMENT_WEIGHT * _item_adjustment(example.get("item_content")) - BLEND_WEIGHT * difficulty
        examples.append((label, offset))
    if not examples:
        return prediction
    examples = examples[-POSTERIOR_MAX_LABELS:]
    theta_prior = _ability_prior_logit(input)
    theta = theta_prior
    for _ in range(max(1, POSTERIOR_MAX_STEPS)):
        grad = -POSTERIOR_PRIOR_PRECISION * (theta - theta_prior)
        hess = -POSTERIOR_PRIOR_PRECISION
        for label, offset in examples:
            p = _sigmoid(theta + offset)
            grad += label - p
            hess -= p * (1.0 - p)
        if abs(hess) < 1e-8:
            break
        ...
        if abs(step) < 1e-4:
            break
    target_offset = ITEM_ADJUSTMENT_WEIGHT * _item_adjustment(input.get("item_content")) - BLEND_WEIGHT * target_difficulty
    return _clamp(_sigmoid(theta + target_offset))
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    prediction = _posterior_subject_prediction(prediction, input, difficulty, labeled)
