"""BGE-large ridge with anchored model-capability interactions.
...
Capability correction: smoothed model bucket offsets interact with item bucket loadings.
"""
...
CAPABILITY_WEIGHT = float(ARTIFACT.get("capability_weight", 0.0))
BUCKET_NAMES = [str(x) for x in ARTIFACT.get("bucket_names", [])]
CAPABILITY_OFFSETS = {
    str(subject).strip().lower(): {str(bucket): float(value) for bucket, value in offsets.items()}
    for subject, offsets in ARTIFACT.get("capability_offsets", {}).items()
}
...
def _bucket_loadings(benchmark: object, item_content: object) -> dict[str, float]:
    names = BUCKET_NAMES or list(BUCKET_KEYWORDS)
    ...
    if not any(values.values()) and "general_reasoning" in values:
        values["general_reasoning"] = 0.5
    return values


def _capability_correction(input: dict) -> float:
    if not CAPABILITY_WEIGHT or not CAPABILITY_OFFSETS:
        return 0.0
    subject_name = _parse_subject_name(input.get("subject_content"))
    offsets = CAPABILITY_OFFSETS.get(subject_name)
    if not offsets:
        return 0.0
    loadings = _bucket_loadings(input.get("benchmark"), input.get("item_content"))
    return sum(float(offsets.get(bucket, 0.0)) * loading for bucket, loading in loadings.items())
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    capability = _capability_correction(input)
    if difficulty is None:
        prediction = _sigmoid(_logit(base) + CAPABILITY_WEIGHT * capability)
    else:
        prediction = _sigmoid(_logit(base) - BLEND_WEIGHT * difficulty + CAPABILITY_WEIGHT * capability)
