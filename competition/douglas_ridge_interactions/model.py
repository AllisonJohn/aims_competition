...
USE_INTERACTION_FEATURES = bool(ARTIFACT.get("interaction_features", False))
INTERACTION_NUMERIC_INDICES = [
    int(i)
    for i in ARTIFACT.get("interaction_numeric_indices", [3, 4, 5, 6, 7])
]
...
def _predict_centered_difficulty(item_content: object) -> float | None:
    ...
    if USE_INTERACTION_FEATURES:
        features.extend(_interaction_features(embedding, numeric))
    return RIDGE_INTERCEPT + sum(coef * value for coef, value in zip(RIDGE_COEF, features))


def _interaction_features(embedding: list[float], numeric: list[float]) -> list[float]:
    features = []
    ...
    for index in INTERACTION_NUMERIC_INDICES:
        if 0 <= index < len(numeric):
            value = numeric[index]
            features.extend(value * component for component in embedding)
    return features
