...
LOCAL_TYPE_WEIGHT = 0.20
LOCAL_TYPE_TAU = 2.0
LOCAL_TYPE_CAP = 0.15
LOCAL_TYPE_PRIOR = 2.0
SUBJECT_FACTORS = {
...
def _factor_logit(input: dict) -> float | None:
    ...
    return max(-LOGIT_CAP, min(LOGIT_CAP, item_bias + sum(u * v for u, v in zip(subject_factors, item_factors))))


def _local_type_features(input: dict) -> list[float] | None:
    if not CALIBRATOR:
        return None
    factor_logit = _factor_logit(input)
    if factor_logit is None:
        return None
    raw = _calibrator_features(input, factor_logit)
    mean = [float(x) for x in CALIBRATOR["scaler_mean"]]
    scale = [float(x) if float(x) != 0.0 else 1.0 for x in CALIBRATOR["scaler_scale"]]
    return [(value - mu) / sigma for value, mu, sigma in zip(raw, mean, scale)]


def _local_type_delta(input: dict, labeled: list[dict] | None) -> float:
    if not labeled:
        return 0.0
    current = _local_type_features(input)
    if current is None:
        return 0.0
    benchmark = _key(input.get("benchmark"))
    numerator = 0.0
    denominator = float(LOCAL_TYPE_PRIOR)
    dim = max(1, len(current))
    for example in labeled:
        if "label" not in example or _key(example.get("benchmark")) != benchmark:
            continue
        other = _local_type_features(example)
        if other is None:
            continue
        p = _raw_predict(example)
        y = _clamp(float(example["label"]), 0.0, 1.0)
        dist2 = sum((a - b) * (a - b) for a, b in zip(current, other)) / dim
        weight = math.exp(-dist2 / float(LOCAL_TYPE_TAU))
        residual = (y - p) / max(0.05, p * (1.0 - p))
        numerator += weight * residual
        denominator += weight
    if denominator <= LOCAL_TYPE_PRIOR:
        return 0.0
    delta = numerator / denominator
    return max(-LOCAL_TYPE_CAP, min(LOCAL_TYPE_CAP, delta))
...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    ...
    delta += LOCAL_TYPE_WEIGHT * _local_type_delta(input, labeled)
