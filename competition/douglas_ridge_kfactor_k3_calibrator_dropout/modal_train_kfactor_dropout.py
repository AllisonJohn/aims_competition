...
app = modal.App("cs321m-final-bge-kfactor-k3-calibrator-dropout-artifact")
...
def train_remote(
    ...
    dropout_copies: int = 2,
    subject_dropout: float = 0.2,
    condition_dropout: float = 0.5,
    benchmark_dropout: float = 0.8,
    adjustment_dropout: float = 0.3,
) -> dict:
    import math
    ...
    def invert_dropout_column(
        ...
        if dropout_rate <= 0.0:
            return values.copy()
        if dropout_rate >= 1.0:
            return np.full_like(values, float(neutral_value), dtype=np.float32)
        keep_prob = 1.0 - dropout_rate
        keep = rng.binomial(1, keep_prob, size=values.shape[0]).astype(np.float32)
        return (neutral_value + keep * (values - neutral_value) / keep_prob).astype(np.float32)

    def augment_calibrator_features(features: np.ndarray, labels: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        if dropout_copies <= 0:
            return features, labels
        rng = np.random.default_rng(326)
        global_logit = logit(global_rate)
        augmented = [features]
        for _ in range(int(dropout_copies)):
            dropped = features.copy()
            factor = dropped[:, 4].copy()
            dropped[:, 0] = invert_dropout_column(dropped[:, 0], global_logit, subject_dropout, rng)
            dropped[:, 1] = invert_dropout_column(dropped[:, 1], global_logit, condition_dropout, rng)
            dropped[:, 2] = invert_dropout_column(dropped[:, 2], global_logit, benchmark_dropout, rng)
            dropped[:, 3] = invert_dropout_column(dropped[:, 3], 0.0, adjustment_dropout, rng)
            dropped[:, 5] = dropped[:, 0] * factor
            dropped[:, 6] = dropped[:, 1] * factor
            dropped[:, 7] = dropped[:, 2] * factor
            dropped[:, 8] = np.abs(factor)
            augmented.append(dropped.astype(np.float32))
        return np.vstack(augmented).astype(np.float32), np.tile(labels, int(dropout_copies) + 1)

    calibration_x, calibration_y = augment_calibrator_features(calibration_x, calibration_y)
    ...
    return {
        "artifact": {
            ...
            "calibrator": {
                ...
                "c": float(calibrator_c),
                "dropout": {
                    "mode": "inverted_feature_dropout",
                    "copies": int(dropout_copies),
                    "subject_dropout": float(subject_dropout),
                    "condition_dropout": float(condition_dropout),
                    "benchmark_dropout": float(benchmark_dropout),
                    "adjustment_dropout": float(adjustment_dropout),
                    "neutral_logit": float(logit(global_rate)),
                    "neutral_adjustment": 0.0,
                    "factor_logit_dropout": 0.0,
                },
            },
        ...
        "summary": {
            ...
            "calibrator_rows": int(calibration_x.shape[0]),
