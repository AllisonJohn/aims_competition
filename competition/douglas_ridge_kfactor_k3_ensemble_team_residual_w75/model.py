...
def _calibrate_benchmark_only(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    if not labeled:
        return prediction
    benchmark = _key(input.get("benchmark"))
    if not benchmark:
    ...
    for example in labeled:
        ...
        if _key(example.get("benchmark")) != benchmark:
            continue
        labels.append(_clamp(float(example["label"]), 0.0, 1.0))
    ...
    posterior_rate = _clamp(
        (prior_rate * BENCHMARK_BAYES_PRIOR_COUNT + observed_rate * len(labels))
        / (BENCHMARK_BAYES_PRIOR_COUNT + len(labels))
    )
    delta = _logit(posterior_rate) - _logit(prior_rate)
    delta = max(-BAYES_DELTA_CAP, min(BAYES_DELTA_CAP, delta))
    return _clamp(_sigmoid(_logit(prediction) + delta))


def predict(input: dict, labeled: list[dict] | None = None) -> float:
    prediction = _raw_predict(input)
    bayes_prediction = _calibrate_benchmark_only(prediction, input, labeled)
    residual_prediction = _calibrate_with_labeled(prediction, input, labeled)
    blended_logit = (
        (1.0 - TEAM_RESIDUAL_WEIGHT) * _logit(bayes_prediction)
        + TEAM_RESIDUAL_WEIGHT * _logit(residual_prediction)
    )
