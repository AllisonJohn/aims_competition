...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    ...
    residual_delta = 0.65 * category_delta + 0.45 * subject_delta
    coherence = 0
    if benchmark_delta * category_delta >= 0.0:
        coherence += 1
    if benchmark_delta * subject_delta >= 0.0:
        coherence += 1
    if category_delta * subject_delta >= 0.0:
        coherence += 1
    if coherence >= 2:
        residual_scale = 1.0
    elif coherence == 1:
        ...
        residual_scale = 0.0
    total_delta = benchmark_delta + residual_scale * residual_delta
