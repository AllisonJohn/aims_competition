...
app = modal.App("cs321m-final-bge-kfactor-k3-final-pca-artifact")
...
def train_remote(
    ...
    pca_components = min(int(final_pca_components), x_train.shape[0] - 1, x_train.shape[1])
    if pca_components < 1:
        pca_components = 1
    final_pca = PCA(n_components=pca_components, svd_solver="randomized", random_state=326)
    item_pca = final_pca.fit_transform(x_train).astype(np.float32)
    item_pca_by_key = {
        str(item_key): item_pca[index]
        for index, item_key in enumerate(item_targets["item_key"])
    }
    zero_pca = np.zeros(pca_components, dtype=np.float32)
    print(
        f"final_calibrator_pca_components={pca_components} "
        f"explained_variance={float(final_pca.explained_variance_ratio_.sum()):.4f}",
        flush=True,
    )
    ...
    for row_index, row in enumerate(train_df.itertuples(index=False)):
        ...
        text_pca = item_pca_by_key.get(str(row.item_key), zero_pca)
        calibration_rows.append(
            ...
            + [float(value) for value in text_pca]
        )
    ...
    return {
        "artifact": {
            ...
            "calibrator": {
                ...
                + [f"text_pca_{index}" for index in range(pca_components)],
            ...
            "final_pca": {
                "components": [[float(value) for value in row] for row in final_pca.components_],
                "mean": [float(value) for value in final_pca.mean_],
                "explained_variance_ratio": [float(value) for value in final_pca.explained_variance_ratio_],
        ...
        "summary": {
            ...
            "final_pca_components": int(pca_components),
