"""BGE-large + K-factor IRT ridge + final PCA-text calibrator submission.
...
Final calibrator also receives PCA-reduced item text features directly.
"""
...
FINAL_PCA = ARTIFACT.get("final_pca")
...
def _final_pca_features(features: list[float] | None) -> list[float]:
    if not FINAL_PCA or features is None:
        return []
    mean = [float(x) for x in FINAL_PCA["mean"]]
    components = FINAL_PCA["components"]
    centered = [
        float(value) - (mean[index] if index < len(mean) else 0.0)
        for index, value in enumerate(features)
    ]
    projected = []
    for row in components:
        projected.append(sum(float(weight) * value for weight, value in zip(row, centered)))
    return projected
...
def _predict_item_latents_and_features(item_content: object) -> tuple[list[float], float, list[float]] | None:
    features = _feature_vector(item_content)
    if features is None:
        return None
    values = {}
    ...
    item_factors = [values.get(f"factor_{k}", 0.0) for k in range(LATENT_DIM)]
    item_bias = values.get("item_bias", 0.0)
    return item_factors, item_bias, _final_pca_features(features)
...
def _calibrator_features(input: dict, factor_logit: float, text_pca: list[float] | None = None) -> list[float]:
    subject_name = _parse_subject_name(input.get("subject_content"))
    condition = _key(input.get("condition") or "none")
    benchmark = _key(input.get("benchmark"))
    subject_rate = _smooth(SUBJECT_RATES.get(subject_name), SUBJECT_ALPHA)
    ...
    if text_pca:
        features.extend(float(value) for value in text_pca)
    return features


def _learned_factor_prediction(input: dict, factor_logit: float, text_pca: list[float] | None = None) -> float | None:
    if not CALIBRATOR:
        return None
    features = _calibrator_features(input, factor_logit, text_pca)
...
def _uncalibrated_prediction(input: dict) -> float:
    ...
    latents = _predict_item_latents_and_features(input.get("item_content"))
    if latents is None:
        return base
    item_factors, item_bias, text_pca = latents
    ...
    learned_prediction = _learned_factor_prediction(input, factor_logit, text_pca)
