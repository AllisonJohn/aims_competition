...
app = modal.App("cs321m-final-bge-kfactor-k3-lobo-sweep")
sweep_volume = modal.Volume.from_name("aims-k3-lobo-sweep-results", create_if_missing=True)
...
@app.function(gpu="H100", image=image, timeout=60 * 60 * 18, volumes={"/sweep_results": sweep_volume})
def sweep_remote(
    ...
    def checkpoint(stage: str) -> None:
        ...
        payload = {
            ...
            "completed_folds": [row["heldout_benchmark"] for row in fold_summaries],
        ...
        path = Path("/sweep_results") / f"{safe_name}_{stage}.json"
        printable = {
            "checkpoint": str(path),
            "stage": stage,
    ...
    for heldout in heldout_benchmarks:
        ...
        x_train_items_full = train_embeddings.astype(np.float32)
        x_valid_items_full = valid_embeddings.astype(np.float32)
...
def main(
    ...
    if not wait_for_result:
        call = sweep_remote.spawn(*args)
        call_id = getattr(call, "object_id", None) or getattr(call, "call_id", None) or str(call)
        print(f"Spawned sweep call: {call_id}")
        print("Checkpoints will be printed in Modal logs and saved to volume aims-k3-lobo-sweep-results.")
        return

    summary = sweep_remote.remote(*args)
    summary_path = out_dir / f"leave_one_benchmark_sweep_{run_name}.json"
