...
app = modal.App("cs321m-final-bge-kfactor-k3-freeze-subject-refit-item")
...
def train_remote(
    ...
    warm_subject_factors = {
        str(key).strip().lower(): [float(x) for x in value]
        for key, value in warm_artifact["subject_factors"].items()
    }
    warm_global_subject_factor = np.asarray(
        [float(x) for x in warm_artifact.get("global_subject_factor", [0.0] * latent_dim)],
        dtype=np.float32,
    )
    if int(warm_artifact.get("latent_dim", latent_dim)) != int(latent_dim):
        raise ValueError("Warm artifact latent_dim does not match requested latent_dim")
    fixed_subject_factor_matrix = np.vstack(
        [
            np.asarray(
                warm_subject_factors.get(str(name).strip().lower(), warm_global_subject_factor),
                dtype=np.float32,
            )
            for name in subject_uniques
        ]
    ).astype(np.float32)
    print(
        "Freezing warm subject factors: "
        f"matched={sum(str(name).strip().lower() in warm_subject_factors for name in subject_uniques)}"
        f"/{len(subject_uniques)}",
        flush=True,
    )

    item_positive = np.bincount(item_codes, weights=y, minlength=len(item_uniques)).astype(np.float32)
    item_count = np.bincount(item_codes, minlength=len(item_uniques)).astype(np.float32)
    global_rate_init = float(y.mean())
    item_rate = (item_positive + 5.0 * global_rate_init) / (item_count + 5.0)
    item_bias_init = np.asarray([logit(float(rate)) for rate in item_rate], dtype=np.float32)

    class FrozenSubjectKFactorIRT(torch.nn.Module):
        def __init__(self, subject_matrix: np.ndarray, num_items: int, dim: int, bias_init: np.ndarray):
            super().__init__()
            self.register_buffer(
                "subject_factors",
                torch.as_tensor(subject_matrix, dtype=torch.float32),
            )
            ...
            with torch.no_grad():
                self.item_bias.weight.copy_(torch.as_tensor(bias_init[:, None], dtype=torch.float32))
    ...
    irt = FrozenSubjectKFactorIRT(
        fixed_subject_factor_matrix,
        len(item_uniques),
        latent_dim,
        item_bias_init,
    ).to(device)
    ...
    with torch.no_grad():
        subject_factor_matrix = fixed_subject_factor_matrix.astype(np.float32)
    ...
    return {
        ...
        "summary": {
            ...
            "irt_lr": float(irt_lr),
            "irt_l2": float(irt_l2),
            "subject_factors_frozen": True,
            "warm_subject_artifact": True,
            "ridge_alpha": float(ridge_alpha),
...
def main(
    ...
    warm_artifact_path = out_dir / "bge_kfactor_ridge_artifact.json"
    out_dir.mkdir(parents=True, exist_ok=True)
    stats = json.loads(stats_path.read_text(encoding="utf-8"))
    warm_artifact = json.loads(warm_artifact_path.read_text(encoding="utf-8"))
