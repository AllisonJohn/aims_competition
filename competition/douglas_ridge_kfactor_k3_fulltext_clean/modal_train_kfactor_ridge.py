...
app = modal.App("cs321m-final-bge-kfactor-k3-clean-id-artifact")
...
def train_remote(
    ...
    def clean_item_text(item_content: object) -> str:
        text = str(item_content or "")
        text = text.replace("\r\n", "\n").replace("\r", "\n")
        text = re.sub(
    ...
    class Embedder:
        ...
        def encode(self, texts: list[str], batch_size: int = 96) -> np.ndarray:
            ...
            for start in range(0, len(texts), batch_size):
                batch = [clean_item_text(text) for text in texts[start : start + batch_size]]
    ...
    return {
        "artifact": {
            ...
            "max_length": 256,
            "text_cleaning": "benchmark_hash_id_only",
        },
        "summary": {
