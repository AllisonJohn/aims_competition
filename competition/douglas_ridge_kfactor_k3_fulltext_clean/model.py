...
USE_TEXT_CLEANING = ARTIFACT.get("text_cleaning") == "benchmark_hash_id_only"
...
def _clean_item_text(item_content: object) -> str:
    text = str(item_content or "")
    if not USE_TEXT_CLEANING:
        return text
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    text = re.sub(
        r"\b(hle|androidworld|bfcl|cybench|mtbench|swebench):[0-9a-f]{8,}\b",
...
def _embed_item(item_content: object) -> list[float] | None:
    text = _clean_item_text(item_content)
