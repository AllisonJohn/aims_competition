...
app = modal.App("cs321m-final-bge-kfactor-k3-gbdt-pca-artifact")
...
def train_remote(
    ...
    tree_l2_regularization: float = 1.0,
    ...
    print(
        f"Training {latent_dim + 1} PCA+GBDT heads for item factors/bias on {len(item_targets)} items",
        flush=True,
    )
    embedder = Embedder()
    ...
    x_train_raw = np.hstack([embeddings, numeric]).astype(np.float32)
    n_components = min(int(pca_components), x_train_raw.shape[0] - 1, x_train_raw.shape[1])
    if n_components < 1:
        n_components = min(x_train_raw.shape[0], x_train_raw.shape[1])
    pca = PCA(n_components=n_components, svd_solver="randomized", random_state=326)
    x_train = pca.fit_transform(x_train_raw).astype(np.float32)
    print(
        f"pca_components={n_components} explained_variance={float(pca.explained_variance_ratio_.sum()):.4f}",
        flush=True,
    )
    ...
    for target_name in target_names:
        ...
        tree = HistGradientBoostingRegressor(
            loss="squared_error",
            max_iter=int(tree_max_iter),
            learning_rate=float(tree_learning_rate),
            l2_regularization=float(tree_l2_regularization),
            max_leaf_nodes=int(tree_max_leaf_nodes),
            early_stopping=True,
            validation_fraction=0.1,
            n_iter_no_change=20,
            random_state=326,
        )
        tree.fit(x_train, target_normalized)
        print(
            f"gbdt_head={target_name} target_mean={target_mean:.6f} "
            f"target_std={target_std:.6f} normalized_train_std={target_normalized.std():.6f} "
            f"iterations={tree.n_iter_}",
            flush=True,
        )
        tree_heads.append(
            {
    ...
    artifact = {
            ...
            "item_model_type": "pca_hist_gradient_boosting",
    ...
    summary = {
        ...
        "tree_heads": int(len(tree_heads)),
        "tree_max_iter": int(tree_max_iter),
        "tree_learning_rate": float(tree_learning_rate),
        "tree_l2_regularization": float(tree_l2_regularization),
        "tree_max_leaf_nodes": int(tree_max_leaf_nodes),
        "targets_standardized": True,
        "blend_weight": float(blend_weight),
        "logit_cap": float(logit_cap),
        "calibrator_features": int(calibration_x.shape[1]),
    }
    return {
        "artifact_bytes": pickle.dumps(artifact, protocol=pickle.HIGHEST_PROTOCOL),
        "summary": {
            **summary,
            "artifact_serialization": "pickle_bytes",
        },
    }
...
def main(
    ...
    payload = train_remote.remote(
        ...
        tree_max_iter,
        tree_learning_rate,
        tree_l2_regularization,
        tree_max_leaf_nodes,
        blend_weight,
        logit_cap,
    )
    with (out_dir / "bge_kfactor_gbdt_pca_artifact.pkl").open("wb") as f:
        f.write(payload["artifact_bytes"])
    (out_dir / "bge_kfactor_gbdt_pca_summary.json").write_text(
        json.dumps(payload["summary"], indent=2, sort_keys=True),
        encoding="utf-8",
    )
    print(json.dumps(payload["summary"], indent=2, sort_keys=True))
    print(f"Wrote {out_dir / 'bge_kfactor_gbdt_pca_artifact.pkl'}")
