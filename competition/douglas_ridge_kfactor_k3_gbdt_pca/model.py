"""BGE-large + K-factor IRT PCA/GBDT submission.

Base predictor: smoothed subject/condition/benchmark baseline.
K-factor correction: BGE-large item text -> PCA -> boosted tree item factor
vector/bias, combined with the learned subject/model factor vector from the
K-dimensional IRT fit.
"""
...
with (ROOT / "artifacts" / "baseline_stats.json").open("r", encoding="utf-8") as f:
    STATS = json.load(f)
with (ROOT / "artifacts" / "bge_kfactor_gbdt_pca_artifact.pkl").open("rb") as f:
...
TREE_HEADS = ARTIFACT["tree_heads"]
NUMERIC_SCALER = ARTIFACT["numeric_scaler"]
PCA_MODEL = ARTIFACT["pca"]
...
def _feature_vector(item_content: object) -> list[float] | None:
    ...
    numeric_scaled = NUMERIC_SCALER.transform(np.asarray([numeric], dtype=np.float32))
    raw = np.hstack([np.asarray([embedding], dtype=np.float32), numeric_scaled]).astype(np.float32)
    return PCA_MODEL.transform(raw).astype(float)[0].tolist()
...
def _predict_item_latents(item_content: object) -> tuple[list[float], float] | None:
    ...
    for head in TREE_HEADS:
        prediction = float(head["model"].predict(x)[0])
