...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    ...
    def bayes_delta(group_labels: list[float], group_expected: list[float], prior_count: float, cap: float, smooth_observed: bool = False) -> float:
        if not group_labels:
            return 0.0
        prior_rate = _clamp(sum(group_expected) / len(group_expected))
        if smooth_observed:
            observed_rate = _clamp((sum(group_labels) + 0.5) / (len(group_labels) + 1.0))
        else:
    ...
    category_delta = bayes_delta(category_labels, category_expected, 8.0, CATEGORY_DELTA_CAP, True)
    subject_delta = bayes_delta(subject_labels, subject_expected, 6.0, SUBJECT_DELTA_CAP, True)
