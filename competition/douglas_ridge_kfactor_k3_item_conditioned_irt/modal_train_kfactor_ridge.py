...
app = modal.App("cs321m-final-bge-kfactor-k3-item-conditioned-irt-artifact")
...
def train_remote(
    ...
    item_epochs: int = 5,
    item_distill_epochs: int = 2,
    item_lr: float = 1e-3,
    item_weight_decay: float = 1e-4,
    ...
    print(
        f"Training item-conditioned IRT head on {len(item_targets)} items "
        f"hidden={item_hidden_dim} item_epochs={item_epochs} distill_epochs={item_distill_epochs}",
        flush=True,
    )
    embedder = Embedder()
    ...
    target_matrix = item_targets[target_names].to_numpy(dtype=np.float32)

    class ItemConditionedIRT(torch.nn.Module):
        def __init__(self, num_subjects: int, input_dim: int, dim: int, hidden_dim: int):
            super().__init__()
            self.subject_factors = torch.nn.Embedding(num_subjects, dim)
            self.item_net = torch.nn.Sequential(
                torch.nn.Linear(input_dim, hidden_dim),
                torch.nn.ReLU(),
                torch.nn.Linear(hidden_dim, hidden_dim),
                torch.nn.ReLU(),
                torch.nn.Linear(hidden_dim, dim + 1),
            )

        def forward(self, subjects_tensor, item_features):
            u = self.subject_factors(subjects_tensor)
            item_params = self.item_net(item_features)
            v = item_params[:, :latent_dim]
            b = item_params[:, latent_dim]
            return (u * v).sum(dim=1) + b, u, v, item_params

    item_key_to_target_index = {
        str(key): index for index, key in enumerate(item_targets["item_key"].astype(str).tolist())
    }
    row_item_target = train_df["item_key"].astype(str).map(item_key_to_target_index)
    row_mask = row_item_target.notna().to_numpy()
    if int(row_mask.sum()) == 0:
        raise ValueError("No training rows matched encoded item targets.")
    fit_subject_codes = subject_codes[row_mask].astype(np.int64)
    fit_item_codes = row_item_target[row_mask].to_numpy(dtype=np.int64)
    fit_y = y[row_mask].astype(np.float32)
    calib_df = train_df.loc[row_mask].reset_index(drop=True)

    item_feature_tensor = torch.as_tensor(x_train, dtype=torch.float32, device=device)
    target_tensor = torch.as_tensor(target_matrix, dtype=torch.float32, device=device)
    item_model = ItemConditionedIRT(
        len(subject_uniques),
        int(x_train.shape[1]),
        latent_dim,
        item_hidden_dim,
    ...
    with torch.no_grad():
        item_model.subject_factors.weight.copy_(torch.as_tensor(subject_factor_matrix, device=device))

    if item_distill_epochs > 0:
        distill_dataset = TensorDataset(torch.arange(len(item_targets), dtype=torch.long))
        distill_loader = DataLoader(distill_dataset, batch_size=2048, shuffle=True)
        distill_optimizer = torch.optim.AdamW(
            item_model.item_net.parameters(),
            lr=item_lr,
            weight_decay=item_weight_decay,
        )
        mse = torch.nn.MSELoss()
        for epoch in range(1, item_distill_epochs + 1):
            running = 0.0
            seen = 0
            for (items_batch,) in distill_loader:
                items_batch = items_batch.to(device, non_blocking=True)
                prediction = item_model.item_net(item_feature_tensor[items_batch])
                loss = mse(prediction, target_tensor[items_batch])
                distill_optimizer.zero_grad(set_to_none=True)
                loss.backward()
                distill_optimizer.step()
                batch_size = int(items_batch.numel())
                running += float(loss.detach().cpu()) * batch_size
                seen += batch_size
            print(f"item_distill_epoch={epoch} mse={running / max(seen, 1):.6f}", flush=True)

    item_dataset = TensorDataset(
        torch.as_tensor(fit_subject_codes, dtype=torch.long),
        torch.as_tensor(fit_item_codes, dtype=torch.long),
        torch.as_tensor(fit_y, dtype=torch.float32),
    )
    ...
    item_optimizer = torch.optim.AdamW(item_model.parameters(), lr=item_lr, weight_decay=item_weight_decay)
    for epoch in range(1, item_epochs + 1):
        running_loss = 0.0
        seen = 0
        for batch_index, (subjects_batch, items_batch, labels_batch) in enumerate(item_loader, start=1):
            subjects_batch = subjects_batch.to(device, non_blocking=True)
            items_batch = items_batch.to(device, non_blocking=True)
            labels_batch = labels_batch.to(device, non_blocking=True)
            logits, u_batch, v_batch, item_params = item_model(subjects_batch, item_feature_tensor[items_batch])
            bce = criterion(logits, labels_batch)
            reg = u_batch.pow(2).mean() + v_batch.pow(2).mean() + item_params[:, latent_dim].pow(2).mean()
            loss = bce + irt_l2 * reg
            ...
            if batch_index % 100 == 0:
                print(
                    f"item_epoch={epoch} batch={batch_index}/{len(item_loader)} "
                    f"batch_bce={float(bce.detach().cpu()):.6f} running_bce={running_loss / seen:.6f}",
                    flush=True,
                )
        print(f"item_epoch={epoch} train_bce={running_loss / max(seen, 1):.6f}", flush=True)

    with torch.no_grad():
        subject_factor_matrix = item_model.subject_factors.weight.detach().cpu().numpy().astype(np.float32)
        item_params = item_model.item_net(item_feature_tensor).detach().cpu().numpy().astype(np.float32)
        item_factor_matrix = item_params[:, :latent_dim]
        item_bias = item_params[:, latent_dim]

    def linear_payload(layer) -> dict:
        return {
            "weight": layer.weight.detach().cpu().numpy().astype(np.float32).tolist(),
            "bias": layer.bias.detach().cpu().numpy().astype(np.float32).tolist(),
        }

    item_net_payload = {
        "input_dim": int(x_train.shape[1]),
        "hidden_dim": int(item_hidden_dim),
        "output_dim": int(latent_dim + 1),
        "layers": [
            linear_payload(item_model.item_net[0]),
            linear_payload(item_model.item_net[2]),
            linear_payload(item_model.item_net[4]),
        ],
    }
    ...
    factor_logits = item_bias[fit_item_codes] + np.sum(
        subject_factor_matrix[fit_subject_codes] * item_factor_matrix[fit_item_codes],
        axis=1,
    )
    ...
    calibration_rows = []
    for row_index, row in enumerate(calib_df.itertuples(index=False)):
    ...
    calibration_y = fit_y.astype(np.int64)
    ...
    return {
        ...
        "summary": {
            ...
            "item_epochs": int(item_epochs),
            "item_distill_epochs": int(item_distill_epochs),
            "item_lr": float(item_lr),
            "item_weight_decay": float(item_weight_decay),
            "ridge_heads": 0,
