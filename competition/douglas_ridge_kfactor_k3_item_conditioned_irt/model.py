...
ONLINE_MAP_L2 = float(ARTIFACT.get("online_map_l2", 2.0))
ONLINE_MAP_STEPS = int(ARTIFACT.get("online_map_steps", 12))
ONLINE_MAP_LR = float(ARTIFACT.get("online_map_lr", 0.35))
ONLINE_MAP_MAX_LABELS = int(ARTIFACT.get("online_map_max_labels", 32))
...
ITEM_NET = ARTIFACT.get("item_net")
...
def _predict_item_latents(item_content: object) -> tuple[list[float], float] | None:
    ...
    if features is None:
        return None
    if ITEM_NET:
        values = _item_net_forward(features)
        if values is None:
            return None
        item_factors = [values[k] for k in range(LATENT_DIM)]
        item_bias = values[LATENT_DIM]
...
def _linear(values: list[float], layer: dict) -> list[float]:
    weights = layer["weight"]
    bias = layer["bias"]
    return [
        float(b) + sum(float(w) * value for w, value in zip(row, values))
        for row, b in zip(weights, bias)
    ]


def _item_net_forward(features: list[float]) -> list[float] | None:
    layers = ITEM_NET.get("layers", []) if ITEM_NET else []
    if not layers:
        return None
    values = features
    for index, layer in enumerate(layers):
        values = _linear(values, layer)
        if index < len(layers) - 1:
            values = _relu(values)
    if len(values) < LATENT_DIM + 1:
        return None
    return values
...
def _factor_logit_for(input: dict, subject_factors: list[float]) -> float | None:
    latents = _predict_item_latents(input.get("item_content"))
    if latents is None:
        return None
    item_factors, item_bias = latents
    factor_logit = item_bias + sum(u * v for u, v in zip(subject_factors, item_factors))
    return max(-LOGIT_CAP, min(LOGIT_CAP, factor_logit))


def _online_map_subject_factor(input: dict, labeled: list[dict] | None) -> list[float] | None:
    if not labeled:
        return None
    subject_name = _parse_subject_name(input.get("subject_content"))
    prior = list(_subject_factor(input.get("subject_content")))
    observations = []
    for example in labeled:
        ...
        try:
            ...
            continue
        latents = _predict_item_latents(example.get("item_content"))
        if latents is None:
            continue
        item_factors, item_bias = latents
        observations.append((label, item_factors, item_bias))
    if not observations:
        return None

    observations = observations[-ONLINE_MAP_MAX_LABELS:]
    u = prior[:]
    for _ in range(max(1, ONLINE_MAP_STEPS)):
        grad = [ONLINE_MAP_L2 * (value - prior_value) for value, prior_value in zip(u, prior)]
        for label, item_factors, item_bias in observations:
            z = item_bias + sum(value * factor for value, factor in zip(u, item_factors))
            z = max(-LOGIT_CAP, min(LOGIT_CAP, z))
            p = _sigmoid(z)
            residual = p - label
            # Fisher weighting: uncertain, high-loading examples should move
            # the subject vector more than saturated or near-zero-loading ones.
            info = p * (1.0 - p) * sum(factor * factor for factor in item_factors)
            weight = max(0.25, min(2.0, 4.0 * info))
            for k, factor in enumerate(item_factors):
                grad[k] += weight * residual * factor
        grad_norm = math.sqrt(sum(value * value for value in grad))
        if grad_norm < 1e-5:
            break
        for k in range(len(u)):
            u[k] -= ONLINE_MAP_LR * grad[k]
            u[k] = max(-3.0, min(3.0, u[k]))
    return u


def predict(input: dict, labeled: list[dict] | None = None) -> float:
    if labeled:
        adapted_factors = _online_map_subject_factor(input, labeled)
        if adapted_factors is not None:
            factor_logit = _factor_logit_for(input, adapted_factors)
            if factor_logit is not None:
                learned_prediction = _learned_factor_prediction(input, factor_logit)
                if learned_prediction is not None:
                    return float(_clamp(learned_prediction))
                base = _base_prediction(input)
                blended_logit = (1.0 - BLEND_WEIGHT) * _logit(base) + BLEND_WEIGHT * factor_logit
                return float(_clamp(_sigmoid(blended_logit)))
