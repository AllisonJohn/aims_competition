...
def _infer_category(benchmark: object, condition: object, item_content: object) -> str:
    ...
    if b == "cybench" or re.search(r"\b(cve|exploit|vulnerability|pwn|crypto|forensics|ctf|shellcode|privilege escalation|xss|sql injection)\b", lower):
        return "cyber"
    if b in {"hle", "mmlupro"}:
        return "knowledge"
    if b == "agentdojo":
...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    ...
    total_delta = benchmark_delta + 0.85 * category_delta + 0.35 * subject_delta
