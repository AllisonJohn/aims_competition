...
app = modal.App("cs321m-final-bge-kfactor-k3-moe-artifact")
...
def train_remote(
    ...
    def fit_ridge_heads(indices: np.ndarray, expert_name: str) -> list[dict]:
        heads = []
        if len(indices) < int(expert_min_items):
            print(
                f"expert={expert_name} has {len(indices)} items; using all items for fallback-stable heads",
                flush=True,
            )
            indices = np.arange(len(item_targets))
        for target_name in target_names:
            target = item_targets[target_name].to_numpy(dtype=np.float32)[indices]
            ...
            ridge.fit(x_train[indices], target_normalized)
            print(
                f"expert={expert_name} ridge_head={target_name} n={len(indices)} "
                f"target_mean={target_mean:.6f} target_std={target_std:.6f}",
                flush=True,
            )
            heads.append(
                {
    ...
    for target_name in target_names:
        ...
        print(
            f"global_ridge_head={target_name} target_mean={target_mean:.6f} "
            f"target_std={target_std:.6f} normalized_train_std={target_normalized.std():.6f}",
            flush=True,
        )
        global_ridge_heads.append(
            {
                "name": target_name,
                "ridge_intercept": float(ridge.intercept_),
                "ridge_coef": [float(x) for x in ridge.coef_],
    ...
    kmeans = KMeans(n_clusters=int(num_experts), random_state=326, n_init=10)
    expert_labels = kmeans.fit_predict(x_train)
    moe_experts = []
    for expert_id in range(int(num_experts)):
        indices = np.where(expert_labels == expert_id)[0]
        moe_experts.append(
            {
                "expert_id": int(expert_id),
                "n_items": int(len(indices)),
                "ridge_heads": fit_ridge_heads(indices, str(expert_id)),
            }
        )
    print(
        "MoE expert sizes: "
        + ", ".join(f"{expert['expert_id']}={expert['n_items']}" for expert in moe_experts),
        flush=True,
    )
    ...
    return {
        "artifact": {
            ...
            "ridge_heads": global_ridge_heads,
            "moe": {
                "num_experts": int(num_experts),
                "gate_temperature": float(gate_temperature),
                "expert_min_items": int(expert_min_items),
                "cluster_centers": kmeans.cluster_centers_.astype(float).tolist(),
                "experts": moe_experts,
            },
        ...
        "summary": {
            ...
            "irt_l2": float(irt_l2),
            "ridge_alpha": float(ridge_alpha),
            "ridge_heads": int(len(global_ridge_heads)),
            "moe_num_experts": int(num_experts),
            "moe_gate_temperature": float(gate_temperature),
            "moe_expert_sizes": [int(expert["n_items"]) for expert in moe_experts],
