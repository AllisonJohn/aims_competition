...
def _predict_item_latents(item_content: object) -> tuple[list[float], float] | None:
    ...
    if MOE:
        centers = MOE.get("cluster_centers", [])
        experts = MOE.get("experts", [])
        temperature = max(float(MOE.get("gate_temperature", 1.0)), 1e-6)
        if centers and experts:
            distances = []
            for center in centers:
                distance = 0.0
                for value, center_value in zip(features, center):
                    diff = value - float(center_value)
                    distance += diff * diff
                distances.append(distance)
            logits = [-distance / temperature for distance in distances]
            max_logit = max(logits)
            weights = [math.exp(logit - max_logit) for logit in logits]
            total = sum(weights) or 1.0
            weights = [weight / total for weight in weights]
            values = {f"factor_{k}": 0.0 for k in range(LATENT_DIM)}
            values["item_bias"] = 0.0
            for expert, gate_weight in zip(experts, weights):
                expert_values = {}
                for head in expert.get("ridge_heads", []):
                    ...
                    expert_values[str(head["name"])] = prediction
                for key in values:
                    values[key] += gate_weight * expert_values.get(key, 0.0)
