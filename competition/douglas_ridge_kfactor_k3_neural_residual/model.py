...
NEURAL_RESIDUAL = CALIBRATOR.get("neural_residual") if CALIBRATOR else None
SUBJECT_FACTORS = {
...
def _learned_factor_prediction(input: dict, factor_logit: float) -> float | None:
    ...
    scaled = [((value - mu) / sigma) for value, mu, sigma in zip(features, mean, scale)]
    z = float(CALIBRATOR["intercept"])
    for value, weight in zip(scaled, coef):
        z += weight * value
    if NEURAL_RESIDUAL:
        hidden = []
        for weights, bias in zip(NEURAL_RESIDUAL["fc1_weight"], NEURAL_RESIDUAL["fc1_bias"]):
            value = float(bias) + sum(float(w) * x for w, x in zip(weights, scaled))
            hidden.append(math.tanh(value))
        residual = float(NEURAL_RESIDUAL["fc2_bias"])
        residual += sum(float(w) * h for w, h in zip(NEURAL_RESIDUAL["fc2_weight"], hidden))
        z += float(NEURAL_RESIDUAL.get("scale", 0.15)) * residual
    return _sigmoid(z)
