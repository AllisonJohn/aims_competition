...
app = modal.App("cs321m-final-bge-kfactor-k3-normalized-loadings")
...
def train_remote(
    ...
    item_loading_norm = np.linalg.norm(item_factor_matrix, axis=1).astype(np.float32)
    item_loading_norm = np.maximum(item_loading_norm, 1e-6).astype(np.float32)
    item_loading_direction = (item_factor_matrix / item_loading_norm[:, None]).astype(np.float32)
    log_item_loading_norm = np.log(item_loading_norm).astype(np.float32)

    item_targets = pd.DataFrame({"item_key": item_uniques, "item_bias": item_bias})
    for k in range(latent_dim):
        item_targets[f"factor_{k}"] = item_loading_direction[:, k]
    item_targets["log_loading_norm"] = log_item_loading_norm
    ...
    print(
        f"Training {latent_dim + 2} ridge heads for normalized item loading direction/norm/bias "
    ...
    ridge_heads = []
    target_names = [f"factor_{k}" for k in range(latent_dim)] + ["log_loading_norm", "item_bias"]
    ...
    return {
        ...
        "summary": {
            ...
            "latent_dim": int(latent_dim),
            "normalized_item_loadings": True,
            "loading_norm_mean": float(item_loading_norm.mean()),
            "loading_norm_std": float(item_loading_norm.std()),
