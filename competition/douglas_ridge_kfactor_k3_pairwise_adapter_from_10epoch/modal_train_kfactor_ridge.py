...
app = modal.App("cs321m-final-bge-kfactor-k3-pairwise-adapter-10epoch")
...
def train_remote(
    ...
    calibration_logits = (
        ...
        + calibration_x_scaled @ np.asarray(calibrator.coef_[0], dtype=np.float32)
    )
    pairwise_rows = []
    pairwise_y = []
    for row_index, row in enumerate(train_df.itertuples(index=False)):
        is_pairwise, benchmark_flag, cue_score = pairwise_cue_score(
            row.benchmark_id,
            row.condition,
            row.item_content,
        )
        if not is_pairwise:
            continue
        base_features = calibration_x[row_index].tolist()
        pairwise_rows.append(
            [
                float(calibration_logits[row_index]),
                *[float(x) for x in base_features],
                float(benchmark_flag),
                float(cue_score),
            ]
        )
        pairwise_y.append(int(calibration_y[row_index]))
    ...
    pairwise_residual = {
        ...
        "rows": int(len(pairwise_y)),
    }
    if len(pairwise_y) >= 100 and len(set(pairwise_y)) == 2:
    ...
    print(
        "pairwise_residual "
        f"enabled={pairwise_residual.get('enabled')} rows={pairwise_residual.get('rows')} "
        f"weight={pairwise_residual.get('weight')}",
        flush=True,
    )
    ...
    return {
        "artifact": {
            ...
            "pairwise_residual": pairwise_residual,
            "calibrator": {
                "feature_names": [
                    "subject_logit",
                    "condition_logit",
        ...
        "summary": {
            ...
            "pairwise_residual_enabled": bool(pairwise_residual.get("enabled")),
            "pairwise_residual_rows": int(pairwise_residual.get("rows", 0)),
...
def main(
    ...
    raise RuntimeError(
        "This variant freezes the k3 10epoch base artifact. "
        "Run modal_train_pairwise_adapter.py to train only the pairwise adapter."
    )
