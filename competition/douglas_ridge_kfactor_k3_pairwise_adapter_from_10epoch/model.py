...
def _pairwise_adapter_prediction(input: dict, factor_logit: float, prediction: float) -> float | None:
    ...
    for value, mu, sigma, weight in zip(features, mean, scale, coef):
        z += weight * ((value - mu) / sigma)
    adapter_prediction = _sigmoid(z)
    adapter_weight = max(0.0, min(1.0, float(PAIRWISE_RESIDUAL.get("weight", 0.75))))
    blended_logit = (1.0 - adapter_weight) * _logit(prediction) + adapter_weight * _logit(adapter_prediction)
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    if latents is None:
        ...
        pairwise_prediction = _pairwise_adapter_prediction(input, factor_logit, prediction)
