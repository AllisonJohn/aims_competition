...
PAIRWISE_RESIDUAL = ARTIFACT.get("pairwise_residual", {})
SUBJECT_FACTORS = {
...
def _pairwise_cue_score(input: dict) -> tuple[bool, float, float]:
    benchmark = _key(input.get("benchmark"))
    known = {str(x).strip().lower() for x in PAIRWISE_RESIDUAL.get("known_benchmarks", [])}
    keywords = tuple(str(x).lower() for x in PAIRWISE_RESIDUAL.get("keywords", []))
    text = f"{input.get('condition') or ''}\n{input.get('item_content') or ''}".lower()
    benchmark_flag = float(benchmark in known)
    cue_count = sum(1 for keyword in keywords if keyword and keyword in text)
    cue_score = min(1.0, cue_count / 3.0)
    return bool(benchmark_flag or cue_score >= 0.67), benchmark_flag, cue_score


def _pairwise_residual_prediction(input: dict, factor_logit: float, prediction: float) -> float | None:
    if not PAIRWISE_RESIDUAL or not PAIRWISE_RESIDUAL.get("enabled"):
        return None
    is_pairwise, benchmark_flag, cue_score = _pairwise_cue_score(input)
    if not is_pairwise:
        return None
    features = [
        _logit(prediction),
        *_calibrator_features(input, factor_logit),
        benchmark_flag,
        cue_score,
    ]
    mean = [float(x) for x in PAIRWISE_RESIDUAL["scaler_mean"]]
    scale = [float(x) if float(x) != 0.0 else 1.0 for x in PAIRWISE_RESIDUAL["scaler_scale"]]
    coef = [float(x) for x in PAIRWISE_RESIDUAL["coef"]]
    if not (len(features) == len(mean) == len(scale) == len(coef)):
        return None
    z = float(PAIRWISE_RESIDUAL["intercept"])
    for value, mu, sigma, weight in zip(features, mean, scale, coef):
        z += weight * ((value - mu) / sigma)
    pairwise_prediction = _sigmoid(z)
    residual_weight = max(0.0, min(1.0, float(PAIRWISE_RESIDUAL.get("weight", 0.75))))
    blended_logit = (1.0 - residual_weight) * _logit(prediction) + residual_weight * _logit(pairwise_prediction)
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    if latents is None:
        ...
        pairwise_prediction = _pairwise_residual_prediction(input, factor_logit, prediction)
        if pairwise_prediction is not None:
