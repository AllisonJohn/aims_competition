...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    ...
    category_weight = 0.65 if category_delta * benchmark_delta >= 0.0 else 0.25
    subject_weight = 0.45 if subject_delta * benchmark_delta >= 0.0 else 0.18
    total_delta = benchmark_delta + category_weight * category_delta + subject_weight * subject_delta
