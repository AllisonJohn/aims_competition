...
BENCHMARK_BAYES_PRIOR_COUNT = 11.0
BAYES_DELTA_CAP = 0.35
CATEGORY_DELTA_CAP = 0.18
SUBJECT_DELTA_CAP = 0.16
SUBJECT_FACTORS = {
...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    ...
    if not benchmark:
        return prediction
    input_subject = _parse_subject_name(input.get("subject_content"))
    input_category = _infer_category(input.get("benchmark"), input.get("condition"), input.get("item_content"))
    input_condition = _key(input.get("condition") or "none")
    labels = []
    expected = []
    category_labels = []
    category_expected = []
    ...
    for example in labeled:
        ...
        expected_value = _raw_predict(example)
        labels.append(label)
        expected.append(expected_value)
        if _infer_category(example.get("benchmark"), example.get("condition"), example.get("item_content")) == input_category:
            category_labels.append(label)
            category_expected.append(expected_value)
            category_match = True
        else:
            category_match = False
        if _parse_subject_name(example.get("subject_content")) == input_subject:
            subject_labels.append(label)
            subject_expected.append(expected_value)
            if category_match:
                cat_subject_labels.append(label)
                cat_subject_expected.append(expected_value)
        if _key(example.get("condition") or "none") == input_condition:
            condition_labels.append(label)
            condition_expected.append(expected_value)
    if not labels:
        return prediction

    def bayes_delta(group_labels: list[float], group_expected: list[float], prior_count: float, cap: float) -> float:
        if not group_labels:
            return 0.0
        prior_rate = _clamp(sum(group_expected) / len(group_expected))
        observed_rate = _clamp(sum(group_labels) / len(group_labels))
        posterior_rate = _clamp(
            (prior_rate * prior_count + observed_rate * len(group_labels))
            / (prior_count + len(group_labels))
        )
        delta_value = _logit(posterior_rate) - _logit(prior_rate)
        return max(-cap, min(cap, delta_value))

    benchmark_delta = bayes_delta(labels, expected, BENCHMARK_BAYES_PRIOR_COUNT, BAYES_DELTA_CAP)
    category_delta = bayes_delta(category_labels, category_expected, 8.0, CATEGORY_DELTA_CAP)
    subject_delta = bayes_delta(subject_labels, subject_expected, 6.0, SUBJECT_DELTA_CAP)
    condition_delta = bayes_delta(condition_labels, condition_expected, 8.0, 0.12)
    cat_subject_delta = bayes_delta(cat_subject_labels, cat_subject_expected, 5.0, 0.14)
    category_weight = 0.55 * len(category_labels) / (len(category_labels) + 1.0)
    subject_weight = 0.35 * len(subject_labels) / (len(subject_labels) + 1.0)
    if category_delta * benchmark_delta < 0.0:
        category_weight *= 0.45
    if subject_delta * benchmark_delta < 0.0:
        subject_weight *= 0.45
    total_delta = (
        benchmark_delta
        + category_weight * category_delta
        + subject_weight * subject_delta
        + 0.15 * condition_delta
        + 0.25 * cat_subject_delta
    )
    return _clamp(_sigmoid(_logit(prediction) + total_delta))
