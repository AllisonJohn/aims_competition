...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    ...
    residual_delta = 0.65 * category_delta
    if abs(0.45 * subject_delta) > abs(residual_delta):
        residual_delta = 0.45 * subject_delta
    total_delta = benchmark_delta + residual_delta
