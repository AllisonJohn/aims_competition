...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    ...
    category_delta = bayes_delta(category_labels, category_expected, 8.0, CATEGORY_DELTA_CAP) if len(category_labels) >= 2 else 0.0
    subject_delta = bayes_delta(subject_labels, subject_expected, 6.0, SUBJECT_DELTA_CAP) if len(subject_labels) >= 2 else 0.0
