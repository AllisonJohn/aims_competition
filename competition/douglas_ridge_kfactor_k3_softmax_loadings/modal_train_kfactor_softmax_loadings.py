...
app = modal.App("cs321m-final-bge-kfactor-k3-softmax-loadings")
...
def train_remote(
    ...
    print(
        f"Training {latent_dim + 1} ridge heads for item loading logits/bias "
        f"on {len(item_targets)} items",
        flush=True,
    )
    embedder = Embedder()
    ...
    for target_name in target_names:
        ...
        prediction = ridge.predict(x_train).astype(np.float32)
        ridge_predictions[target_name] = (target_mean + target_std * prediction).astype(np.float32)
        print(
    ...
    raw_factor_scores = np.stack(
        [ridge_predictions[f"factor_{k}"] for k in range(latent_dim)],
        axis=1,
    )
    raw_factor_scores = raw_factor_scores - raw_factor_scores.max(axis=1, keepdims=True)
    softmax_weights = np.exp(np.clip(raw_factor_scores, -40.0, 40.0)).astype(np.float32)
    softmax_weights /= softmax_weights.sum(axis=1, keepdims=True).clip(min=1e-12)
    predicted_item_factors = softmax_weights.astype(np.float32)
    predicted_item_bias = ridge_predictions["item_bias"].astype(np.float32)
    item_position_by_key = {str(key): index for index, key in enumerate(item_targets["item_key"])}
    predicted_item_index = np.asarray(
        [item_position_by_key.get(str(key), -1) for key in train_df["item_key"]],
        dtype=np.int64,
    )
    row_item_factors = np.zeros((len(train_df), latent_dim), dtype=np.float32)
    row_item_bias = np.zeros(len(train_df), dtype=np.float32)
    known_rows = predicted_item_index >= 0
    row_item_factors[known_rows] = predicted_item_factors[predicted_item_index[known_rows]]
    row_item_bias[known_rows] = predicted_item_bias[predicted_item_index[known_rows]]
    print(
        f"Calibration uses ridge-predicted softmax item loadings for "
        f"{int(known_rows.sum())}/{len(train_df)} rows",
        flush=True,
    )
    factor_logits = row_item_bias + np.sum(
        subject_factor_matrix[subject_codes] * row_item_factors,
        axis=1,
    )
    ...
    return {
        "artifact": {
            ...
            "normalized_item_loadings": False,
            "softmax_item_loadings": True,
