"""BGE-large + softmax-normalized K-factor IRT ridge submission.

Base predictor: smoothed subject/condition/benchmark baseline.
K-factor correction: BGE-large item text -> predicted item loading weights,
loading norm, and bias, combined with learned subject/model factors.
"""
...
def _predict_item_latents(item_content: object) -> tuple[list[float], float] | None:
    ...
    if ARTIFACT.get("softmax_item_loadings"):
        max_value = max(item_factors) if item_factors else 0.0
        exp_values = [math.exp(max(-40.0, min(40.0, value - max_value))) for value in item_factors]
        total = sum(exp_values)
        if total > 1e-12:
            item_factors = [value / total for value in exp_values]
    elif ARTIFACT.get("normalized_item_loadings"):
        norm = math.sqrt(sum(value * value for value in item_factors))
        if norm > 1e-6:
            item_factors = [value / norm for value in item_factors]
        loading_norm = math.exp(max(-6.0, min(3.0, values.get("log_loading_norm", 0.0))))
        item_factors = [loading_norm * value for value in item_factors]
