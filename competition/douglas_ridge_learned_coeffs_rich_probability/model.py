"""BGE-large + rich learned probability submission.
...
Final probability: learned logistic model over prior logits, difficulty, and
...
with (ROOT / "artifacts" / "baseline_stats.json").open("r", encoding="utf-8") as f:
    STATS = json.load(f)
with (ROOT / "artifacts" / "bge_rich_probability_artifact.json").open("r", encoding="utf-8") as f:
...
def _calibrator_features(input: dict, difficulty: float) -> list[float]:
    ...
    base_prob = _sigmoid(max(-20.0, min(20.0, base_logit)))
    return [
        subject_logit,
        condition_logit,
        benchmark_logit,
