"""BGE-large + centered IRT difficulty correction with Bayesian 1D IRT adapter.
...
At prediction time, labeled examples update a scalar ability by MAP.
"""
...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    ...
    for _ in range(12):
        ...
        if hess <= 1e-12:
            break
        theta -= grad / hess
    return _clamp(_sigmoid(theta - float(target_difficulty)))
