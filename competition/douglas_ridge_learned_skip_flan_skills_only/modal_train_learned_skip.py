...
app = modal.App("cs321m-final-bge-flan-skills-only-artifact")
...
def train_remote(
    ...
    embedding_mode: str = "skills_only",
    ...
    if embedding_mode not in {"item_skills", "skills_only"}:
        raise ValueError("embedding_mode must be 'item_skills' or 'skills_only'")
    ...
    def parse_skill_tags(text: str) -> list[str]:
        ...
        for tag in SKILL_TAGS:
            if tag in lower.replace("-", "_").replace(" ", "_"):
                found.append(tag)
        if found:
            return found
        rough = {
    ...
    def skill_features(tag_lists: list[list[str]]) -> np.ndarray:
        index = {tag: idx for idx, tag in enumerate(SKILL_TAGS)}
        matrix = np.zeros((len(tag_lists), len(SKILL_TAGS)), dtype=np.float32)
        for row_idx, tags in enumerate(tag_lists):
            for tag in tags:
                if tag in index:
                    matrix[row_idx, index[tag]] = 1.0
        return matrix
    ...
    class SkillExtractor:
        def __init__(self):
            self.tokenizer = AutoTokenizer.from_pretrained(flan_model_id)
            self.model = AutoModelForSeq2SeqLM.from_pretrained(flan_model_id).to("cuda")
            self.model.eval()

        @torch.no_grad()
        def extract(self, texts: list[str], batch_size: int = 32) -> list[list[str]]:
            outputs = []
            ...
            for start in range(0, len(texts), batch_size):
                ...
                outputs.extend(parse_skill_tags(text) for text in decoded)
                print(f"extracted skills {min(start + len(batch), len(texts))}/{len(texts)}", flush=True)
            return outputs
    ...
    item_texts = item_targets["item_content"].fillna("").astype(str).tolist()
    skill_extractor = SkillExtractor()
    skill_tag_lists = skill_extractor.extract(item_texts)
    skill_texts = [", ".join(tags) for tags in skill_tag_lists]
    skill_matrix = skill_features(skill_tag_lists)
    if embedding_mode == "skills_only":
        enhanced_texts = [f"Required skills: {skill_text}" for skill_text in skill_texts]
    else:
        enhanced_texts = [
            f"{item_text}\n\nRequired skills: {skill_text}"
            for item_text, skill_text in zip(item_texts, skill_texts)
        ]
    embedder = Embedder()
    embeddings = embedder.encode(enhanced_texts)
    scaler = StandardScaler()
    numeric = scaler.fit_transform(numeric_features(item_targets))
    x_train = np.hstack([embeddings, numeric, skill_matrix]).astype(np.float32)
    ...
    return {
        "artifact": {
            ...
            "skill_dim": int(skill_matrix.shape[1]),
            "max_length": 256,
            "flan_max_length": 512,
        },
        "summary": {
