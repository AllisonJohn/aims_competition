...
app = modal.App("cs321m-final-bge-learned-skip-mlp-head-deep-256-64")
...
def train_remote(
    ...
    item_head_hidden_dim: int = 256,
    item_head_hidden2_dim: int = 64,
    ...
    class ItemMLPHead(torch.nn.Module):
        def __init__(self, input_dim: int, hidden_dim: int, hidden2_dim: int):
            super().__init__()
            self.linear1 = torch.nn.Linear(input_dim, hidden_dim)
            self.linear2 = torch.nn.Linear(hidden_dim, hidden2_dim)
            self.linear3 = torch.nn.Linear(hidden2_dim, 1)

        def forward(self, features: torch.Tensor) -> torch.Tensor:
            hidden = F.leaky_relu(self.linear1(features), negative_slope=0.1)
            hidden = F.leaky_relu(self.linear2(hidden), negative_slope=0.1)
            return self.linear3(hidden)

    device = "cuda" if torch.cuda.is_available() else "cpu"
    item_head = ItemMLPHead(
        x_train.shape[1],
        int(item_head_hidden_dim),
        int(item_head_hidden2_dim),
    ).to(device)
    ...
    print(
        ...
        f"hidden={item_head_hidden_dim}->{item_head_hidden2_dim} epochs={item_head_epochs} lr={item_head_lr} "
    ...
    return {
        "artifact": {
            ...
            "item_head": {
                "type": "linear_leakyrelu_linear_leakyrelu_linear",
                ...
                "hidden2_dim": int(item_head_hidden2_dim),
                "output_dim": 1,
                "target_mean": [0.0],
                "target_scale": [1.0],
                ...
                "linear3_weight": item_head.linear3.weight.detach().cpu().numpy().astype(float).tolist(),
                "linear3_bias": item_head.linear3.bias.detach().cpu().numpy().astype(float).tolist(),
            },
        ...
        "summary": {
            ...
            "item_head_hidden2_dim": int(item_head_hidden2_dim),
