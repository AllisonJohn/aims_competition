...
ITEM_HEAD_LINEAR3_WEIGHT = [
    [float(x) for x in row] for row in ITEM_HEAD.get("linear3_weight", [])
]
ITEM_HEAD_LINEAR3_BIAS = [float(x) for x in ITEM_HEAD.get("linear3_bias", [])]
...
def _predict_centered_difficulty(item_content: object) -> float | None:
    ...
    if ITEM_HEAD_LINEAR3_WEIGHT:
        hidden = [value if value >= 0.0 else 0.1 * value for value in outputs]
        outputs = []
        for weights, bias in zip(ITEM_HEAD_LINEAR3_WEIGHT, ITEM_HEAD_LINEAR3_BIAS):
            outputs.append(bias + sum(weight * value for weight, value in zip(weights, hidden)))
