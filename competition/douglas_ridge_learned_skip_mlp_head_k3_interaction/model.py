"""BGE-large + centered IRT difficulty MLP correction submission.

Base predictor: smoothed subject/condition baseline.
Text correction: BGE-large embedding -> centered IRT item difficulty MLP head.
...
INTERACTION_MODEL = ARTIFACT.get("interaction_model") or {}
INTERACTION_RIDGE_HEADS = INTERACTION_MODEL.get("ridge_heads") or []
INTERACTION_SUBJECT_FACTORS = {
    str(key).strip().lower(): [float(x) for x in value]
    for key, value in (INTERACTION_MODEL.get("subject_factors") or {}).items()
}
INTERACTION_DEFAULT_SUBJECT_FACTOR = [
    float(x) for x in INTERACTION_MODEL.get("default_subject_factor", [])
]
...
def _predict_centered_difficulty(item_content: object) -> float | None:
    ...
    return float(outputs[0] * ITEM_HEAD_TARGET_SCALE[0] + ITEM_HEAD_TARGET_MEAN[0])


def _predict_interaction_logit(input: dict) -> float:
    if not INTERACTION_RIDGE_HEADS or not INTERACTION_DEFAULT_SUBJECT_FACTOR:
        return 0.0
    embedding = _embed_item(input.get("item_content"))
    if embedding is None or len(embedding) != EMBEDDING_DIM:
        return 0.0
    numeric = _numeric_features(input.get("item_content"))
    numeric = [
        (value - SCALER_MEAN[i]) / SCALER_SCALE[i]
        for i, value in enumerate(numeric)
    ]
    features = embedding + numeric
    item_vector = []
    for head in INTERACTION_RIDGE_HEADS:
        z = float(head["ridge_intercept"])
        coef = [float(x) for x in head["ridge_coef"]]
        z += sum(weight * value for weight, value in zip(coef, features))
        item_vector.append(z * float(head["target_std"]) + float(head["target_mean"]))
    subject_name = _parse_subject_name(input.get("subject_content"))
    subject_vector = INTERACTION_SUBJECT_FACTORS.get(subject_name, INTERACTION_DEFAULT_SUBJECT_FACTOR)
    if len(subject_vector) != len(item_vector):
        return 0.0
    return float(sum(s * i for s, i in zip(subject_vector, item_vector)))


def _calibrator_features(input: dict, difficulty: float, interaction_logit: float) -> list[float]:
    subject_name = _parse_subject_name(input.get("subject_content"))
    condition = _key(input.get("condition") or "none")
    benchmark = _key(input.get("benchmark"))
    subject_rate = _smooth(SUBJECT_RATES.get(subject_name), SUBJECT_ALPHA)
...
def _learned_skip_prediction(input: dict, difficulty: float, interaction_logit: float) -> float | None:
    if not CALIBRATOR:
        return None
    features = _calibrator_features(input, difficulty, interaction_logit)
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    if difficulty is None:
        ...
        interaction_logit = _predict_interaction_logit(input)
        learned_prediction = _learned_skip_prediction(input, difficulty, interaction_logit)
