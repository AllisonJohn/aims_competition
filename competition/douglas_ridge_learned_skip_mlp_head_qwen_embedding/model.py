    ...
    try:
        with TORCH.no_grad():
            ...
            if "qwen" in MODEL_ID.lower():
                positions = TORCH.arange(encoded["attention_mask"].shape[1], device=DEVICE)
                sequence_lengths = (encoded["attention_mask"] * positions).argmax(dim=1)
                batch_indices = TORCH.arange(output.shape[0], device=output.device)
                pooled = output[batch_indices, sequence_lengths]
            else:
