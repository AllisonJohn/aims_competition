...
app = modal.App("cs321m-final-bge-learned-skip-modified-artifact")
...
def train_remote(
    ...
    actual_topic_count = max(1, min(int(topic_count), len(item_targets)))
    kmeans = KMeans(n_clusters=actual_topic_count, random_state=326, n_init=10)
    topic_ids = kmeans.fit_predict(embeddings)
    item_targets["topic_id"] = topic_ids
    topic_by_item = dict(zip(item_targets["item_key"], item_targets["topic_id"]))
    train_df["topic_id"] = train_df["item_key"].map(topic_by_item)
    ...
    topic_rates = rate_table(train_df.dropna(subset=["topic_id"]).assign(topic_id=lambda df: df["topic_id"].astype(int)), "topic_id")
    ...
    for row in train_df.itertuples(index=False):
        ...
        topic_key = "" if pd.isna(row.topic_id) else str(int(row.topic_id))
        topic_rate = smooth(topic_rates.get(topic_key), topic_alpha, global_rate)
        subject_logit = logit(subject_rate)
        condition_logit = logit(condition_rate)
        topic_logit = logit(topic_rate)
    ...
    return {
        "artifact": {
            ...
            "topic_prior": {
                "topic_count": int(actual_topic_count),
                "topic_alpha": float(topic_alpha),
                "topic_centers": [[float(x) for x in row] for row in kmeans.cluster_centers_],
                "topic_rates": {
                    str(key): [float(value[0]), float(value[1])]
                    for key, value in topic_rates.items()
                },
            },
            "calibrator": {
                "feature_names": [
