"""BGE-large + unseen-safe learned skip submission.

Base predictor: smoothed subject/condition baseline.
Text correction: BGE-large embedding -> raw IRT item difficulty ridge model.
Benchmark identity is intentionally not used; a transferable topic prior is
inferred from item-text embedding clusters.
"""
...
TOPIC_PRIOR = ARTIFACT.get("topic_prior", {})
TOPIC_CENTERS = TOPIC_PRIOR.get("topic_centers", [])
TOPIC_RATES = TOPIC_PRIOR.get("topic_rates", {})
TOPIC_ALPHA = float(TOPIC_PRIOR.get("topic_alpha", 5000.0))
...
def _topic_logit(embedding: list[float] | None) -> float:
    if embedding is None or not TOPIC_CENTERS:
        return _logit(GLOBAL_RATE)
    best_topic = 0
    best_dist = None
    for idx, center in enumerate(TOPIC_CENTERS):
        if len(center) != len(embedding):
            continue
        dist = 0.0
        for value, center_value in zip(embedding, center):
            diff = float(value) - float(center_value)
            dist += diff * diff
        if best_dist is None or dist < best_dist:
            best_dist = dist
            best_topic = idx
    topic_rate = _smooth(TOPIC_RATES.get(str(best_topic)), TOPIC_ALPHA)
    return _logit(topic_rate)


def _predict_difficulty(item_content: object) -> float | None:
...
def _calibrator_features(input: dict, difficulty: float) -> list[float]:
    ...
    topic_logit = _topic_logit(_embed_item(input.get("item_content")))
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    difficulty = _predict_difficulty(input.get("item_content"))
