...
Run:
    modal run competition/douglas_ridge_logistic/modal_validate_bge_ridge_logistic.py
"""
...
APP_NAME = "douglas-ridge-logistic-validation"
...
image = (
    ...
    .add_local_dir(LOCAL_RIDGE_DIR, remote_path="/root/competition/douglas_ridge_logistic")
)
...
def validate_remote(
    ...
    def smooth(rate_count: list[float] | None, alpha: float) -> float:
        if not rate_count:
            return float(baseline_stats["global_rate"])
    ...
    predicted_difficulties = ridge.predict(x_train)
    difficulty_by_item = {
        item_key: float(difficulty)
        for item_key, difficulty in zip(item_targets["item_key"], predicted_difficulties)
    }
    ...
    for row in train_df.itertuples(index=False):
        subject_rate = smooth(subject_rates.get(str(row.subject_name).strip().lower()), 250.0)
        condition_rate = smooth(condition_rates.get(str(row.condition).strip().lower()), 2000.0)
        benchmark_rate = smooth(benchmark_rates.get(str(row.benchmark_id).strip().lower()), 5000.0)
        difficulty = max(
            -difficulty_cap,
            min(difficulty_cap, difficulty_by_item.get(row.item_key, 0.0)),
        )
    ...
    x_calibration = np.asarray(calibration_rows, dtype=np.float32)
    y_calibration = train_df["label"].to_numpy(dtype=np.int64)
    calibrator = LogisticRegression(C=calibrator_c, max_iter=1000, solver="lbfgs")
    ...
    remote_submission_dir = Path("/root/competition/douglas_ridge_logistic")
    ...
    summary = {
        ...
        "ridge_alpha": 300.0,
        "difficulty_cap": float(difficulty_cap),
        "calibrator_features": artifact["calibrator_features"],
        "calibrator_intercept": float(calibrator.intercept_[0]),
        "calibrator_coef": [float(x) for x in calibrator.coef_[0]],
        "calibrator_c": float(calibrator_c),
    }
...
def main(
    ...
    summary_path = LOCAL_ARTIFACT_DIR / "validation_80_20_logistic_summary.json"
