...
CALIBRATOR_INTERCEPT = ARTIFACT.get("calibrator_intercept")
CALIBRATOR_COEF = ARTIFACT.get("calibrator_coef")
if CALIBRATOR_INTERCEPT is not None and CALIBRATOR_COEF is not None:
    CALIBRATOR_INTERCEPT = float(CALIBRATOR_INTERCEPT)
    CALIBRATOR_COEF = [float(x) for x in CALIBRATOR_COEF]
...
def _base_prediction(input: dict) -> float:
    subject_logit, condition_logit, benchmark_logit, adjustment = _base_components(input)
    z = (
        0.68 * subject_logit
...
def _base_components(input: dict) -> tuple[float, float, float, float]:
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    if CALIBRATOR_INTERCEPT is not None and CALIBRATOR_COEF is not None:
        subject_logit, condition_logit, benchmark_logit, adjustment = _base_components(input)
        clipped_difficulty = 0.0 if difficulty is None else max(
            -DIFFICULTY_CAP,
            min(DIFFICULTY_CAP, difficulty),
        )
        features = [
            subject_logit,
            condition_logit,
        ...
        z = CALIBRATOR_INTERCEPT + sum(
            coef * value for coef, value in zip(CALIBRATOR_COEF, features)
        )
