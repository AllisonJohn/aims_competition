"""Validate the composite handcrafted-text-feature ridge submission with unseen benchmark holdout.

This script trains the same artifact family as ``modal_train_text_features_composite.py``
...
Run:
    modal run competition/douglas_ridge_text_features_composite/modal_validate_text_features_composite.py
"""
...
APP_NAME = "douglas-ridge-text-features-composite-validation"
...
image = (
    ...
    .add_local_dir(LOCAL_RIDGE_DIR, remote_path="/root/competition/douglas_ridge_text_features_composite")
)
...
def validate_remote(
    ...
    remote_submission_dir = Path("/root/competition/douglas_ridge_text_features_composite")
    ...
    x_train = np.asarray(
        [feature_module._composite_features(row.tolist()) for row in base_train],
        dtype=np.float32,
    )
...
def main(
    ...
    summary_path = LOCAL_ARTIFACT_DIR / "validation_80_20_text_features_composite_summary.json"
