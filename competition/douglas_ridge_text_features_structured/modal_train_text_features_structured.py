...
app = modal.App("cs321m-final-text-features-structured-artifact")
...
def train_remote(
    ...
    def expanded_features(raw: np.ndarray, scaled: np.ndarray) -> np.ndarray:
        ...
        interactions = [
            (scaled[:, i] * scaled[:, j])[:, None]
            for i, j in structured_pairs
        ]
    ...
    raw_train = text_features(item_targets)
    base_train = scaler.fit_transform(raw_train).astype(np.float32)
    x_train = expanded_features(raw_train, base_train)
    ...
    return {
        "artifact": {
            ...
            "continuous_transform_count": int(len(continuous_indices) * 3),
            "interaction_count": int(len(structured_pairs)),
        },
        "summary": {
