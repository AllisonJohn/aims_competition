"""Validate the structured handcrafted-text-feature ridge submission with unseen benchmark holdout.

This script trains the same artifact family as ``modal_train_text_features_structured.py``
...
Run:
    modal run competition/douglas_ridge_text_features_structured/modal_validate_text_features_structured.py
"""
...
APP_NAME = "douglas-ridge-text-features-structured-validation"
...
image = (
    ...
    .add_local_dir(LOCAL_RIDGE_DIR, remote_path="/root/competition/douglas_ridge_text_features_structured")
)
...
def validate_remote(
    ...
    remote_submission_dir = Path("/root/competition/douglas_ridge_text_features_structured")
    ...
    dummy_artifact = {
        "encoder_id": "handcrafted-text-features-structured-v1",
        "blend_weight": 0.30,
        "difficulty_cap": 2.5,
        "ridge_heads": [
            {
    ...
    base_train = scaler.fit_transform(raw_features).astype(np.float32)
    x_train = np.asarray(
        [
            feature_module._expanded_features(raw_row.tolist(), scaled_row.tolist())
            for raw_row, scaled_row in zip(raw_features, base_train)
        ],
        dtype=np.float32,
    )
    ...
    for head_index, seed in enumerate(seeds):
        ...
        ridge.fit(x_train[sample_indices], y_train[sample_indices])
        ridge_heads.append(
            {
    ...
    artifact = {
        ...
        "structured_expansion": True,
        "continuous_transform_count": int(len(feature_module.CONTINUOUS_INDICES) * 3),
        "interaction_count": int(len(feature_module.STRUCTURED_INTERACTION_PAIRS)),
    }
    ...
    summary = {
        ...
        "encoder": "handcrafted-text-features-structured-v1",
...
def main(
    ...
    summary_path = LOCAL_ARTIFACT_DIR / "validation_80_20_text_features_structured_summary.json"
