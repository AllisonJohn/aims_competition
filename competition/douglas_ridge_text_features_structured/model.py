"""Structured handcrafted text features + centered IRT difficulty correction submission.

Base predictor: smoothed subject/condition baseline.
Text correction: 73 deterministic item text features plus related polynomial
and interaction terms -> centered IRT item difficulty ridge model.
"""
...
USE_STRUCTURED_EXPANSION = bool(ARTIFACT.get("structured_expansion", False))
...
COMPLEXITY_INDICES = [0, 1, 2, 3, 4, 5, 6, 7, 10, 62, 63, 64, 65, 66, 67, 68, 72]
DOMAIN_INDICES = list(range(26, 51))
MATH_INDICES = [24, 25, 26, 27, 28, 29, 30, 31, 54, 67, 68, 69]
CODE_INDICES = [19, 20, 21, 32, 33, 34, 35, 36, 51, 53, 56, 57, 58, 59]
VISUAL_INDICES = [43, 44]
CHOICE_INDICES = [45, 69]
REASONING_INDICES = [27, 28, 29, 30, 31, 72]
FORMAT_INDICES = [5, 6, 7, 17, 18, 19, 20, 21, 51, 52, 53, 54, 55, 56, 57, 59]
...
def _expanded_features(raw_features: list[float], scaled_features: list[float]) -> list[float]:
    if not USE_STRUCTURED_EXPANSION:
        return scaled_features
    transformed = []
    for index in CONTINUOUS_INDICES:
        raw = max(0.0, raw_features[index])
        scaled = scaled_features[index]
        transformed.append(scaled * scaled)
        transformed.append(math.sqrt(raw))
        transformed.append(math.log1p(10.0 * raw) / math.log1p(10.0))
    interactions = [
        scaled_features[i] * scaled_features[j]
        for i, j in STRUCTURED_INTERACTION_PAIRS
    ]
    return scaled_features + transformed + interactions


def _predict_centered_difficulty(item_content: object) -> float | None:
    raw_features = _text_features(item_content)
    if len(raw_features) != BASE_FEATURE_DIM:
        return None
    scaled_features = [
        (value - SCALER_MEAN[i]) / SCALER_SCALE[i]
        for i, value in enumerate(raw_features)
    ]
    features = _expanded_features(raw_features, scaled_features)
