...
app = modal.App("aims-label-condition-subject-sweep")
...
def tune_remote(
    ...
    grid = []
    for prior in (9.0, 10.0, 11.0):
        for cond_weight, subj_weight in (
            (0.0, 0.0),
            (0.35, 0.0),
            (0.0, 0.25),
            (0.20, 0.10),
    ...
    for config in grid:
        ...
        def calibrate(prediction, input, labeled, cfg=config, m=model):
            if not labeled:
                return prediction
            delta = m._benchmark_rate_delta(input, labeled)
            delta += float(cfg["cond_weight"]) * m._residual_delta(
                input,
                labeled,
                "condition",
                float(cfg["cond_precision"]),
                0.12,
            )
            delta += float(cfg["subj_weight"]) * m._residual_delta(
                input,
                labeled,
                "subject",
                float(cfg["subj_precision"]),
                0.16,
            )
            delta = max(-float(cfg["total_cap"]), min(float(cfg["total_cap"]), delta))
        ...
        print(
            "config "
            f"prior={config['prior']} cond={config['cond_weight']} subj={config['subj_weight']} "
    ...
    return {"results": results, "top": results[:8]}
...
def main(
    ...
    json_out: str | None = "label_validation_sandbox/condition_subject_sweep.json",
) -> None:
