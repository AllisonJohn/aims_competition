...
CONDITION_BAYES_PRIOR_PRECISION = 12.0
SUBJECT_BAYES_PRIOR_PRECISION = 8.0
SUBJECT_FACTORS = {
...
def _benchmark_rate_delta(input: dict, labeled: list[dict] | None) -> float:
    if not labeled:
        return 0.0
    benchmark = _key(input.get("benchmark"))
    if not benchmark:
...
def _residual_delta(
    ...
    if not labeled:
        return 0.0
    if match_key == "condition":
        target = _key(input.get("condition") or "none")
        matches = lambda example: _key(example.get("condition") or "none") == target
    elif match_key == "subject":
        target = _parse_subject_name(input.get("subject_content"))
        matches = lambda example: _parse_subject_name(example.get("subject_content")) == target
    else:
        return 0.0

    numerator = 0.0
    denominator = float(prior_precision)
    used = 0
    for example in labeled:
        if "label" not in example or not matches(example):
            continue
        label = _clamp(float(example["label"]), 0.0, 1.0)
        expected = _raw_predict(example)
        numerator += label - expected
        denominator += expected * (1.0 - expected)
        used += 1
    if used == 0:
        return 0.0
...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    if not labeled:
        return prediction
    delta = _benchmark_rate_delta(input, labeled)
    delta += 0.50 * _residual_delta(
        input,
        labeled,
        "condition",
        CONDITION_BAYES_PRIOR_PRECISION,
        0.12,
    )
    delta += 0.25 * _residual_delta(
        input,
        labeled,
        "subject",
        SUBJECT_BAYES_PRIOR_PRECISION,
        0.16,
    )
    delta = max(-0.42, min(0.42, delta))
