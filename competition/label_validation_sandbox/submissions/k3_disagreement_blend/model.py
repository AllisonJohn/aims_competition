...
def _raw_prediction_parts(input: dict) -> tuple[float, list[float] | None, float | None]:
    base = _base_prediction(input)
    latents = _predict_item_latents(input.get("item_content"))
    if latents is None:
        return float(_clamp(base)), None, None
    ...
    return float(_clamp(prediction)), item_factors, factor_logit


def _raw_predict(input: dict) -> float:
    prediction, _, _ = _raw_prediction_parts(input)
    return prediction
...
def _theta_update_logit_delta(input: dict, labeled: list[dict] | None) -> float:
    if not labeled:
        return 0.0
    subject_name = _parse_subject_name(input.get("subject_content"))
    current_prediction, current_item_factors, _ = _raw_prediction_parts(input)
    if current_item_factors is None:
        return 0.0

    regularizer = 8.0
    fisher = [[regularizer if i == j else 0.0 for j in range(3)] for i in range(3)]
    ...
    for example in labeled:
        ...
        if _parse_subject_name(example.get("subject_content")) != subject_name:
            continue
        p, item_factors, _ = _raw_prediction_parts(example)
        ...
        residual = label - p
        for i in range(3):
            gradient[i] += residual * item_factors[i]
            for j in range(3):
                fisher[i][j] += variance * item_factors[i] * item_factors[j]
        used += 1
    if used == 0:
        return 0.0
    delta_theta = _solve_3x3(fisher, gradient)
    delta = sum(delta_theta[k] * current_item_factors[k] for k in range(3))
    return max(-0.45, min(0.45, delta))
...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    ...
    for example in labeled:
        if "label" not in example:
            continue
        same_subject = _parse_subject_name(example.get("subject_content")) == subject_name
        same_benchmark = benchmark and _key(example.get("benchmark")) == benchmark
        if not same_subject and not same_benchmark:
            continue
        label = _clamp(float(example["label"]), 0.0, 1.0)
        raw = _raw_predict(example)
        base = _base_prediction(example)
        weight = (1.0 if same_subject else 0.25) + (0.25 if same_benchmark else 0.0)
        raw_errors.append(abs(label - raw))
        base_errors.append(abs(label - base))
        weights.append(weight)
    if not weights:
        return prediction
    raw_error = sum(w * e for w, e in zip(weights, raw_errors)) / sum(weights)
    base_error = sum(w * e for w, e in zip(weights, base_errors)) / sum(weights)
    if base_error >= raw_error:
        return prediction
    advantage = min(0.35, max(0.0, raw_error - base_error))
    base_current = _base_prediction(input)
    blend = min(0.30, 0.08 + 1.5 * advantage)
    return _clamp((1.0 - blend) * prediction + blend * base_current)
