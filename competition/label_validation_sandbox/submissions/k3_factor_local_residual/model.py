...
def _factor_cosine(left: list[float] | None, right: list[float] | None) -> float:
    ...
    if left_norm <= 1e-9 or right_norm <= 1e-9:
        return 0.0
    return max(0.0, min(1.0, sum(x * y for x, y in zip(left, right)) / (left_norm * right_norm)))
...
def _theta_update_logit_delta(input: dict, labeled: list[dict] | None) -> float:
    ...
    for example in labeled:
        ...
        same_condition = _key(example.get("condition") or "none") == condition
        same_benchmark = benchmark and _key(example.get("benchmark")) == benchmark
        similarity = _factor_cosine(current_item_factors, item_factors)
        weight = 0.0
        if same_subject:
            weight += 0.75
        if same_condition:
