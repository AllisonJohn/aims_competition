...
def _item_flags(item_content: object) -> set[str]:
    text = str(item_content or "")
    lower = text.lower()
    flags = set()
...
def _length_bucket(item_content: object) -> str:
    length = len(str(item_content or ""))
    if length < 300:
        return "short"
    if length < 1500:
        return "medium"
...
def _item_relatedness(left: object, right: object) -> float:
    left_embedding = _embed_item(left)
    right_embedding = _embed_item(right)
    cosine = _cosine_similarity(left_embedding, right_embedding)

    left_flags = _item_flags(left)
    right_flags = _item_flags(right)
    flag_overlap = len(left_flags & right_flags) / max(1, len(left_flags | right_flags))
    length_match = float(_length_bucket(left) == _length_bucket(right))
    heuristic = 0.75 * flag_overlap + 0.25 * length_match
    return max(cosine, heuristic)
...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    ...
    for example in labeled:
        ...
        relatedness = _item_relatedness(input.get("item_content"), example.get("item_content"))
        ...
        if same_benchmark:
            weight += 0.05
        weight *= 0.25 + 0.75 * relatedness
        if not same_subject:
            weight *= 0.25
        if weight < 0.04:
            continue

        expected = _raw_predict(example)
        numerator += weight * (label - expected)
