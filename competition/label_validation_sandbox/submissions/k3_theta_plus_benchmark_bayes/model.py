...
BENCHMARK_BAYES_PRIOR_COUNT = 20.0
SUBJECT_FACTORS = {
...
def _benchmark_bayes_logit_delta(input: dict, labeled: list[dict] | None) -> float:
    if not labeled:
        return 0.0
    benchmark = _key(input.get("benchmark"))
    if not benchmark:
...
def _calibrate_with_labeled(prediction: float, input: dict, labeled: list[dict] | None) -> float:
    delta = _theta_update_logit_delta(input, labeled) + _benchmark_bayes_logit_delta(input, labeled)
    if delta == 0.0:
        return prediction
